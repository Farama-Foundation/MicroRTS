...
public class FixedOpponentsTournament {
    ...
    public static void runTournament(List<AI> AIs, List<AI> opponentAIs, List<String> maps,
        int iterations, int maxGameLength, int timeBudget, int iterationsBudget,
        long preAnalysisBudgetFirstTimeInAMap, long preAnalysisBudgetRestOfTimes,
        boolean fullObservability, boolean timeoutCheck, boolean runGC, boolean preAnalysis,
        UnitTypeTable utt, String traceOutputfolder, Writer out, Writer progress,
        String folderForReadWriteFolders) throws Exception {
        if (progress != null) {
            progress.write("FixedOpponentsTournament: Starting tournament\n");
        }
        ...
        for (int i = 0; i < opponentAIs.size(); i++) {
            opponentReadWriteFolders[i] =
                folderForReadWriteFolders + "/opponentAI" + i + "readWriteFolder";
                ...
                for (int ai1_idx = 0; ai1_idx < AIs.size(); ai1_idx++) {
                    for (int ai2_idx = 0; ai2_idx < opponentAIs.size(); ai2_idx++) {
                        ...
                        if (crashed != -1) {
                            ...
                            if (timedout == 1) {
                                opponentAItimeout[ai1_idx][ai2_idx]++;
                            }
                        } else {
                            winner = gs.winner();
                        }
