...
public class RoundRobinTournament {
    ...
    public static void runTournament(List<AI> AIs, int playOnlyGamesInvolvingThisAI,
        List<String> maps, int iterations, int maxGameLength, int timeBudget, int iterationsBudget,
        long preAnalysisBudgetFirstTimeInAMap, long preAnalysisBudgetRestOfTimes,
        boolean fullObservability, boolean selfMatches, boolean timeoutCheck, boolean runGC,
        boolean preAnalysis, UnitTypeTable utt, String traceOutputfolder, Writer out,
        Writer progress, String folderForReadWriteFolders) throws Exception {
        if (progress != null) {
        ...
        out.write("pregameAnalysisBudget\t" + preAnalysisBudgetFirstTimeInAMap + "\t"
                ...
                for (int ai1_idx = 0; ai1_idx < AIs.size(); ai1_idx++) {
                    for (int ai2_idx = 0; ai2_idx < AIs.size(); ai2_idx++) {
                        ...
                        if (USE_CONTINUING_ON_INTERRUPTIBLE) {
                            if (ai1 instanceof InterruptibleAI) {
                                ai1 = new ContinuingAI(ai1);
                            }
                        ...
                        if (visualize) {
                            w = PhysicalGameStatePanel
                                .newVisualizer(gs, 600, 600, !fullObservability);
                        }
                        ...
                        if (preAnalysis) {
                            ...
                            ai1.preGameAnalysis(gs, preAnalysisBudgetRestOfTimes,
                                readWriteFolders[ai1_idx]);
                            long pre_end1 = System.currentTimeMillis();
                            if (progress != null) {
                                progress.write(
                                    "preGameAnalysis player 1 took " + (pre_end1 - pre_start1)
                                        + "\n");
                                if ((pre_end1 - pre_start1) > preTime1) {
                                    progress.write("TIMEOUT PLAYER 1!\n");
                                }
                            }
                            if (progress != null) {
                                progress.write(
                                    "preGameAnalysis player 2 took " + (pre_end2 - pre_start2)
                                        + "\n");
                                if ((pre_end2 - pre_start2) > preTime2) {
                                    progress.write("TIMEOUT PLAYER 2!\n");
                                }
                            }
                        }
                        ...
                        do {
                            ...
                            if (fullObservability) {
                                ...
                                try {
                                    PartiallyObservableGameState po_gs = new PartiallyObservableGameState(
                                        gs, 0);
                                    AI1start = System.currentTimeMillis();
                            ...
                            if (traceOutputfolder != null && (!pa1.isEmpty() || !pa2.isEmpty())) {
                                te = new TraceEntry(gs.getPhysicalGameState().clone(),
                                    gs.getTime());
                        ...
                        if (traceOutputfolder != null) {
                            ...
                            String filename =
                                ai1_idx + "-vs-" + ai2_idx + "-" + map_idx + "-" + iteration;
                        ...
                        out.write(
                            iteration + "\t" + map_idx + "\t" + ai1_idx + "\t" + ai2_idx + "\t" + gs
                                .getTime() + "\t" + winner + "\t" + crashed + "\t" + timedout
                                + "\n");
                        out.flush();
                        if (progress != null) {
                            progress
                                .write("Winner: " + winner + "  in " + gs.getTime() + " cycles\n");
                            ...
                            progress.write("AI1 time usage, average:  " + (averageTime1 / numTimes1)
                                ...
                                + ") , # times over 2*budget: " + numberOfTimeOverTwiceBudget1
                                + " (avg " + (averageTimeOverTwiceBudget1
                                / numberOfTimeOverTwiceBudget1) + ")\n");
                            progress.write("AI2 time usage, average:  " + (averageTime2 / numTimes2)
                                ...
                                + ") , # times over 2*budget: " + numberOfTimeOverTwiceBudget2
                                + " (avg " + (averageTimeOverTwiceBudget2
                                / numberOfTimeOverTwiceBudget2) + ")\n");
                        }
                        progress.flush();
                        if (winner == -1) {
