...
public class RoundRobinTournament {
    public static void runTournament(List<AI> AIs,
        ...
        if (progress!=null) progress.write("RoundRobinTournament: Starting tournament\n");
                ...
                for(int ai1_idx = 0;ai1_idx<AIs.size();ai1_idx++) {
                    for(int ai2_idx = 0;ai2_idx<AIs.size();ai2_idx++) {
                        ...
                        if (crashed!=-1) {
                            ...
                            if (crashed==1) AIcrashes[ai2_idx][ai1_idx]++;
                        } else if (timedout!=-1) {
                            winner = 1 - timedout;
                            if (timedout==0) AItimeout[ai1_idx][ai2_idx]++;
                            if (timedout==1) AItimeout[ai2_idx][ai1_idx]++;
                        } else {
                            winner = gs.winner();
                        }
        ...
        out.flush();
        if (progress!=null) progress.write("RoundRobinTournament: tournament ended\n");
        progress.flush();
    }
}
