...
public class FEStatePane extends JPanel {
    ...
    public static Class[] AIs = {PassiveAI.class, MouseController.class, RandomAI.class,
        ...
        POWorkerRush.class, POLightRush.class, POHeavyRush.class, PORangedRush.class,
        WorkerRushPlusPlus.class, CRush_V1.class, CRush_V2.class, PortfolioAI.class, PGSAI.class,
        IDRTMinimax.class, IDRTMinimaxRandomized.class, IDABCD.class, MonteCarlo.class, LSI.class,
    ...
    public static UnitTypeTable[] unitTypeTables = {
        ...
        new UnitTypeTable(UnitTypeTable.VERSION_ORIGINAL,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_RANDOM),
        new UnitTypeTable(UnitTypeTable.VERSION_ORIGINAL_FINETUNED,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_BOTH),
        new UnitTypeTable(UnitTypeTable.VERSION_ORIGINAL_FINETUNED,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_ALTERNATING),
        new UnitTypeTable(UnitTypeTable.VERSION_ORIGINAL_FINETUNED,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_RANDOM),
        new UnitTypeTable(UnitTypeTable.VERSION_NON_DETERMINISTIC,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_BOTH),
        new UnitTypeTable(UnitTypeTable.VERSION_NON_DETERMINISTIC,
            UnitTypeTable.MOVE_CONFLICT_RESOLUTION_CANCEL_ALTERNATING),
        new UnitTypeTable(UnitTypeTable.VERSION_NON_DETERMINISTIC,
    ...
    HashMap[] AIOptionsPanelComponents = {new HashMap<String, JComponent>(),
    ...
    public FEStatePane() throws Exception {
                        ...
                        if (returnVal == JFileChooser.APPROVE_OPTION) {
                            ...
                            try {
                                PhysicalGameState pgs = PhysicalGameState
                                    .load(file.getAbsolutePath(), currentUtt);
            ...
            mapWidthField.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    try {
                        ...
                        statePanel.resizeGameState(newWidth,
                            statePanel.getGameState().getPhysicalGameState().getHeight());
                        statePanel.repaint();
                    } catch (Exception ex) {
                    }
                }
            ...
            mapHeightField.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    try {
                        ...
                        statePanel.resizeGameState(
                            statePanel.getGameState().getPhysicalGameState().getWidth(), newHeight);
                        statePanel.repaint();
                    } catch (Exception ex) {
                    }
                }
                ...
                b.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            ...
                            textArea.setText(
                                "Action generated with " + ai.getClass().getSimpleName() + " in "
                                    + (end - start) + "ms\n");
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            ...
                            for (EvaluationFunction ef : efs) {
                                textArea.append("  - " + ef.getClass().getSimpleName() + ": " + ef
                                    .evaluate(0, 1, statePanel.getGameState()) + ", " + ef
                                    .evaluate(1, 0, statePanel.getGameState()) + "\n");
                            }
                            textArea.append("\n");
                            ...
                            textArea.append("  - player 0: " + BranchingFactorCalculatorBigInteger
                                .branchingFactorByResourceUsageSeparatingFast(
                                    statePanel.getGameState(), 0) + "\n");
                            textArea.append("  - player 1: " + BranchingFactorCalculatorBigInteger
                                .branchingFactorByResourceUsageSeparatingFast(
                                    statePanel.getGameState(), 1) + "\n");
                            textArea.append("\n");
                            ...
                            if (statePanel.getGameState().canExecuteAnyAction(0)) {
                                PlayerActionGenerator pag0 = new PlayerActionGenerator(
                                    statePanel.getGameState(), 0);
                                for (Pair<Unit, List<UnitAction>> tmp : pag0.getChoices()) {
                                    textArea.append(
                                        "    " + tmp.m_a + " has " + tmp.m_b.size() + " actions: "
                                            + tmp.m_b + "\n");
                                }
                                textArea.append("\n");
                            }
                            ...
                            if (statePanel.getGameState().canExecuteAnyAction(1)) {
                                PlayerActionGenerator pag1 = new PlayerActionGenerator(
                                    statePanel.getGameState(), 1);
                        ...
                        Runnable r = new Runnable() {
                            public void run() {
                                try {
                                    ...
                                    JFormattedTextField t1 = (JFormattedTextField) AIOptionsPanelComponents[0]
                                        .get("TimeBudget");
                                    JFormattedTextField t2 = (JFormattedTextField) AIOptionsPanelComponents[1]
                                        .get("TimeBudget");
                                    if (t1 != null) {
                                    ...
                                    if ((ai1 instanceof PseudoContinuingAI)
                                        && (((PseudoContinuingAI) ai1)
                                        .getbaseAI() instanceof MouseController)) {
                                        isMouseController = true;
                                    }
                                    if ((ai2 instanceof PseudoContinuingAI)
                                        && (((PseudoContinuingAI) ai2)
                                        .getbaseAI() instanceof MouseController)) {
                                        isMouseController = true;
                                    }

                                    if (isMouseController) {
                                        PhysicalGameStatePanel pgsp = new PhysicalGameStatePanel(
                                            statePanel);
                                        pgsp.setStateDirect(gs);
                                        w = new PhysicalGameStateMouseJFrame(
                                            "Game State Visualizer (Mouse)", 640, 640, pgsp);

                                        boolean mousep1 = false;
                                        boolean mousep2 = false;
                                        if (ai1 instanceof MouseController) {
                                            ((MouseController) ai1)
                                                .setFrame((PhysicalGameStateMouseJFrame) w);
                                            mousep1 = true;
                                        } else if ((ai1 instanceof PseudoContinuingAI)
                                            && (((PseudoContinuingAI) ai1)
                                            .getbaseAI() instanceof MouseController)) {
                                            ((MouseController) ((PseudoContinuingAI) ai1)
                                                .getbaseAI())
                                                .setFrame((PhysicalGameStateMouseJFrame) w);
                                            mousep1 = true;
                                        }
                                        if (ai2 instanceof MouseController) {
                                            ((MouseController) ai2)
                                                .setFrame((PhysicalGameStateMouseJFrame) w);
                                            mousep2 = true;
                                        } else if ((ai2 instanceof PseudoContinuingAI)
                                            && (((PseudoContinuingAI) ai2)
                                            .getbaseAI() instanceof MouseController)) {
                                            ((MouseController) ((PseudoContinuingAI) ai2)
                                                .getbaseAI())
                                                .setFrame((PhysicalGameStateMouseJFrame) w);
                                            mousep2 = true;
                                        }
                                        ...
                                        w = PhysicalGameStatePanel.newVisualizer(gs, 640, 640,
                                            !fullObservabilityBox.isSelected(),
                                            statePanel.getColorScheme());
                                    }
                                    ...
                                    if (saveTraceBox.isSelected()) {
                                        ...
                                        TraceEntry te = new TraceEntry(
                                            gs.getPhysicalGameState().clone(), gs.getTime());
                                        trace.addEntry(te);
                                    }
                                    ...
                                    do {
                                        if (System.currentTimeMillis() >= nextTimeToUpdate) {
                                            ...
                                            if (fullObservabilityBox.isSelected()) {
                                                ...
                                                pa1 = ai1.getAction(0,
                                                    new PartiallyObservableGameState(gs, 0));
                                                pa2 = ai2.getAction(1,
                                                    new PartiallyObservableGameState(gs, 1));
                                            }
                                            if (trace != null && (!pa1.isEmpty() || !pa2
                                                .isEmpty())) {
                                                //                                                System.out.println("- (for trace) ---------------------------------------");
                                                //                                                System.out.println(gs);
                                                TraceEntry te = new TraceEntry(
                                                    gs.getPhysicalGameState().clone(),
                                                    gs.getTime());
        ...
        for (int player = 0; player < 2; player++) {
            ...
            AIOptionsPanel[player]
                .setLayout(new BoxLayout(AIOptionsPanel[player], BoxLayout.Y_AXIS));
    ...
    public AI createAI(int idx, int player, UnitTypeTable utt) {
        try {
            ...
            for (ParameterSpecification p : parameters) {
                if (p.type == int.class) {
                    JFormattedTextField f = (JFormattedTextField) AIOptionsPanelComponents[player]
                        .get(p.name);
    ...
    private void updateAIOptions(JPanel jPanel, int player) throws Exception {
        ...
        for (ParameterSpecification p : parameters) {
            if (p.type == int.class || p.type == long.class || p.type == float.class
                || p.type == double.class || p.type == String.class) {
                JComponent c = addTextField(jPanel, p.name, p.defaultValue.toString(),
                    p.defaultValue.toString().length() + 1);
                ...
                for (int i = 0; i < pathFinders.length; i++) {
                    ...
                    if (pathFinders[i].getClass() == p.defaultValue.getClass()) {
                        defaultValue = i;
                    }
                }
                ...
                for (int i = 0; i < efs.length; i++) {
                    EFSNames[i] = efs[i];
                    if (efs[i].getClass() == p.defaultValue.getClass()) {
                        defaultValue = i;
                    }
                }
                ...
                if (p.possibleValues == null) {
                    ...
                    for (int i = 0; i < PlayoutAIs.length; i++) {
                        AINames[i] = (AI) PlayoutAIs[i].getConstructor(UnitTypeTable.class)
                            .newInstance(currentUtt);
                        if (PlayoutAIs[i] == p.defaultValue.getClass()) {
                            defaultValue = i;
                        }
                    }
                } else {
                    ...
                    for (int i = 0; i < p.possibleValues.size(); i++) {
                        ...
                        if (p.possibleValues.get(i) == p.defaultValue) {
                            defaultValue = i;
                        }
                    }
                }
                ...
                for (int i = 0; i < p.possibleValues.size(); i++) {
                    ...
                    if (p.possibleValues.get(i).equals(p.defaultValue)) {
                        defaultValue = i;
                    }
                }
