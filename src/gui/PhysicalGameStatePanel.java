...
public class PhysicalGameStatePanel extends JPanel {
    ...
    public static PhysicalGameStateJFrame newVisualizer(GameState a_gs, int dx, int dy,
        boolean a_showVisibility, EvaluationFunction evalFunction, int cs) {
    ...
    public Pair<Integer, Integer> getContentAtCoordinates(int x, int y) {
        ...
        if (cellx >= gs.getPhysicalGameState().getWidth()) {
            return null;
        }
        if (celly >= gs.getPhysicalGameState().getHeight()) {
            return null;
        }
    ...
    public Pair<Integer, Integer> getContentAtCoordinatesBounded(int x, int y) {
        ...
        if (cellx >= gs.getPhysicalGameState().getWidth()) {
            cellx = gs.getPhysicalGameState().getWidth() - 1;
        }
        if (celly >= gs.getPhysicalGameState().getHeight()) {
            celly = gs.getPhysicalGameState().getHeight() - 1;
        }
    ...
    public void paint(Graphics g) {
        ...
        if (gs != null) {
            synchronized (gs) {
                draw(g2d, this, this.getWidth(), this.getHeight(), gs, pogs, colorScheme,
                    fullObservability, drawFromPerspectiveOfPlayer, evalFunction);
            }
        }
    ...
    public static void draw(Graphics2D g2d, PhysicalGameStatePanel panel, int dx, int dy,
        GameState gs, PartiallyObservableGameState[] pogs, int colorScheme,
        boolean fullObservability, int drawFromPerspectiveOfPlayer,
        EvaluationFunction evalFunction) {
        if (gs == null) {
            return;
        }
        ...
        String info =
            "T: " + gs.getTime() + ", P₀: " + unitCount0 + " (" + eval0 + "), P₁: " + unitCount1
                + " (" + eval1 + ")";
        ...
        List<Unit> l = new LinkedList<Unit>(pgs.getUnits());
        for (Unit u : l) {
            int reduction = 0;

            if (!fullObservability && drawFromPerspectiveOfPlayer >= 0
                && !pogs[drawFromPerspectiveOfPlayer].observable(u.getX(), u.getY())) {
                continue;
            }
            ...
            if (uaa != null) {
                ...
                switch (uaa.action.getType()) {
                    case UnitAction.TYPE_MOVE:
                        g2d.setColor(Color.GRAY);
                        g2d.drawLine(u.getX() * grid + grid / 2, u.getY() * grid + grid / 2,
                            u.getX() * grid + grid / 2 + offsx, u.getY() * grid + grid / 2 + offsy);
                        break;
                    ...
                    case UnitAction.TYPE_PRODUCE:
                        ...
                        g2d.fillRect(u.getX() * grid + offsx, u.getY() * grid + offsy,
                            grid - (int) (grid * (((float) ETA) / uaa.action.ETA(uaa.unit))),
                            (int) (grid / 5.0));
                        ...
                        g2d.drawString(txt, u.getX() * grid + grid / 2 - width / 2 + offsx,
                            u.getY() * grid + grid / 2 + offsy);
                        break;
            ...
            if (!u.getType().canMove) {
                g2d.fillRect(u.getX() * grid + reduction, u.getY() * grid + reduction,
                    grid - reduction * 2, grid - reduction * 2);
                g2d.setColor(playerColor);
                if (panel != null && panel.toHighLight.contains(u)) {
                    g2d.setColor(Color.green);
                }
                g2d.drawRect(u.getX() * grid + reduction, u.getY() * grid + reduction,
                    grid - reduction * 2, grid - reduction * 2);
            } else {
                g2d.fillOval(u.getX() * grid + reduction, u.getY() * grid + reduction,
                    grid - reduction * 2, grid - reduction * 2);
                g2d.setColor(playerColor);
                if (panel != null && panel.toHighLight.contains(u)) {
                    g2d.setColor(Color.green);
                }
                g2d.drawOval(u.getX() * grid + reduction, u.getY() * grid + reduction,
                    grid - reduction * 2, grid - reduction * 2);
            }
            ...
            if (u.getType().isStockpile) {
                ...
                FontMetrics fm = g2d.getFontMetrics(g2d.getFont());
                int width = fm.stringWidth(txt);
                g2d.drawString(txt, u.getX() * grid + grid / 2 - width / 2,
                    u.getY() * grid + grid / 2);
            }
            ...
            if (u.getHitPoints() < u.getMaxHitPoints()) {
                ...
                g2d.setColor(Color.GREEN);
                g2d.fillRect(u.getX() * grid, u.getY() * grid,
                    (int) (grid * (((float) u.getHitPoints()) / u.getMaxHitPoints())),
                    (int) (grid / 5.0));
            }
        }
    ...
    public void resizeGameState(int width, int height) {
        if (width >= 1 && height >= 1) {
            ...
            for (int i = 0; i < width * height; i++) {
                newTerrain[i] = PhysicalGameState.TERRAIN_NONE;
            }
