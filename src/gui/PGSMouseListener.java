...
public class PGSMouseListener implements MouseListener, MouseMotionListener, KeyListener {
    ...
    public PGSMouseListener(MouseController a_AI, PhysicalGameStateMouseJFrame a_frame,
        GameState a_gs, int a_playerID) {
        AI = a_AI;
        frame = a_frame;
        gs = a_gs;
        playerID = a_playerID;
                        ...
                        if (rawUnit.getType().isResource) {
                            ...
                            if (selectedUnit.getType().canHarvest) {
                                ...
                                for (Unit u : gs.getPhysicalGameState().getUnits()) {
                                    if (u.getPlayer() == playerID && u.getType().isStockpile) {
                                        double d =
                                            (selectedUnit.getX() - u.getX()) + (selectedUnit.getY()
                                                - u.getY());
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            frame.panel.m_mouse_selection_x0 = frame.panel.m_mouse_selection_x1 =
                e.getX() - insets.left;
            frame.panel.m_mouse_selection_y0 = frame.panel.m_mouse_selection_y1 =
                e.getY() - insets.top;
            frame.repaint();
        }
    }
