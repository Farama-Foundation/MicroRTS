 ...
 * @author santi
 * <p>
 * This class checks whether an AI class satisfies the requirements to be run properly by microRTS.
 * Specifically: - Check that it inherits from the "AI" class - Check that it has at least one
 * constructor with a single parameter (a UnitTypeTable) - Check that it has setters and getters for
 * all the parameters declared in the "getParameters" function
 */
        ...
        try {
            ...
            try {
                ...
                if (cons == null) {
                    System.err.println(c.getName()
                        + " does not have a base constructor with just the UnitTypeTable!");
                    return false;
                }
                AI_instance = (AI) cons.newInstance(utt);
            } catch (java.lang.NoSuchMethodException e) {
                System.err.println(
                    c.getName() + " does not have a base constructor with just the UnitTypeTable!");
                return false;
            }
            ...
            for (ParameterSpecification p : parameters) {
                ...
                try {
                    ...
                    if (getter == null) {
                        System.err.println(
                            c.getName() + " does not have a getter for parameter " + p.name);
                        return false;
                    }
                } catch (java.lang.NoSuchMethodException e) {
                    System.err
                        .println(c.getName() + " does not have a getter for parameter " + p.name);
                    return false;
                }
                ...
                try {
                    ...
                    if (setter == null) {
                        System.err.println(
                            c.getName() + " does not have a setter for parameter " + p.name);
                        return false;
                    }
                } catch (java.lang.NoSuchMethodException e) {
                    System.err
                        .println(c.getName() + " does not have a setter for parameter " + p.name);
                    return false;
                }
            }
