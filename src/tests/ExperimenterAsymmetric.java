...
public class ExperimenterAsymmetric {
    ...
    public static void runExperiments(List<AI> bots1, List<AI> bots2, List<PhysicalGameState> maps,
        ...
        runExperiments(bots1, bots2, maps, utt, iterations, max_cycles, max_inactive_cycles,
            visualize, out, false, false, "");
    }

    public static void runExperiments(List<AI> bots1, List<AI> bots2, List<PhysicalGameState> maps,
        UnitTypeTable utt, int iterations, int max_cycles, int max_inactive_cycles,
        boolean visualize, PrintStream out, boolean saveTrace, boolean saveZip, String traceDir)
        throws Exception {
            ...
            for (int ai2_idx = 0; ai2_idx < bots2.size(); ai2_idx++) {
                ...
                for (PhysicalGameState pgs : maps) {
                    ...
                    for (int i = 0; i < iterations; i++) {
                        ...
                        JFrame w = null;
                        if (visualize) {
                            w = PhysicalGameStatePanel.newVisualizer(gs, 600, 600);
                        }
                        ...
                        do {
                            if (PRINT_BRANCHING_AT_EACH_MOVE) {
                                String bf1 = (gs.canExecuteAnyAction(0) ? ""
                                    + BranchingCalculatorWithTimeOut
                                    .branching(gs, 0, BRANCHING_CALCULATION_TIMEOUT) : "-");
                                String bf2 = (gs.canExecuteAnyAction(1) ? ""
                                    + BranchingCalculatorWithTimeOut
                                    .branching(gs, 1, BRANCHING_CALCULATION_TIMEOUT) : "-");
                        ...
                        if (saveTrace) {
                            ...
                            String filename =
                                ai1.toString() + "Vs" + ai2.toString() + "-" + m + "-" + i;
        ...
        out.println(
            "Notice that the results below are only from the perspective of the 'bots1' list.");
