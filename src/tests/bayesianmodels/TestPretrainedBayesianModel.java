...
public class TestPretrainedBayesianModel {
    ...
    public static void main(String[] args) throws Exception {
        ...
        test(new BayesianModelByUnitTypeWithDefaultModel(new SAXBuilder()
            .build("data/bayesianmodels/pretrained/ActionInterdependenceModel-WR.xml")
            .getRootElement(), utt, new ActionInterdependenceModel(null, 0, 0, 0, utt, fg, ""),
            "AIM_WR"), "data/bayesianmodels/trainingdata/learning-traces-500", "AI0", utt, fg);
    }

    public static void test(BayesianModel model, String tracesFolder, String AIname,
        UnitTypeTable utt, FeatureGenerator fg) throws Exception {
        List<TrainingInstance> instances = PretrainNaiveBayesModels
            .generateInstances(tracesFolder, AIname);
        ...
        System.out.println(
            " /------------ start testing " + tracesFolder + " - " + AIname + " --------------\\ ");
    ...
    public static double crossValidation(BayesianModel model, List<int[]> X_l, List<Integer> Y_l,
        ...
        for (int i = 0; i < X_l.size(); i++) {
            ...
            for (int j = 0; j < nfeatures; j++) {
                if (X_l.get(i)[j] >= Xsizes[j]) {
                    Xsizes[j] = X_l.get(i)[j] + 1;
                }
            }
            if (Y_l.get(i) >= Ysize) {
                Ysize = Y_l.get(i) + 1;
        ...
        for (int fold = 0; fold < nfolds; fold++) {
            ...
            if (DEBUG) {
                System.out
                    .println("  training/test split is " + X_training.size() + "/" + X_test.size());
            }
...
*/
            ...
            for (int i = 0; i < X_test.size(); i++) {
                ...
                if (possibleUnitActions.size() > 1) {
                    ...
                    double[] predicted_distribution = model
                        .predictDistribution(X_test.get(i), i_test.get(i));
                    ...
                    predicted_distribution = model
                        .filterByPossibleActionIndexes(predicted_distribution,
                    ...
                    */
                    ...
                    if (predicted_y == actual_y) {
                        fold_correct_per_unit[u.getType().ID]++;
                    }
            ...
            if (DEBUG) {
                System.out.println(
                    "Average possible actions: " + numPossibleActionsAccum / X_test.size());
            }
            for (int i = 0; i < utt.getUnitTypes().size(); i++) {
                fold_accuracy_per_unit[i] =
                    fold_correct_per_unit[i] / (double) fold_total_per_unit[i];
                if (DEBUG) {
                    System.out.println("Fold accuracy (" + utt.getUnitTypes().get(i).name + "): "
                        + fold_accuracy_per_unit[i] + "   (" + fold_correct_per_unit[i] + "/"
                        + fold_total_per_unit[i] + ")");
                }
            ...
            for (int i = 0; i < utt.getUnitTypes().size(); i++) {
                if (DEBUG) {
                    System.out.println(
                        "Fold loglikelihood (" + utt.getUnitTypes().get(i).name + "): "
                            + fold_loglikelihood_per_unit[i] + " (average: "
                            + fold_loglikelihood_per_unit[i] / fold_total_per_unit[i] + ")");
                }
        ...
        for (int i = 0; i < utt.getUnitTypes().size(); i++) {
            ...
            if (DEBUG) {
                System.out.println(
                    "Final accuracy (" + utt.getUnitTypes().get(i).name + "): " + accuracy_per_unit
                        + "   (" + correct_per_unit[i] + "/" + total_per_unit[i] + ")");
            }
        ...
        for (int i = 0; i < utt.getUnitTypes().size(); i++) {
            if (DEBUG) {
                System.out.println("Final loglikelihood (" + utt.getUnitTypes().get(i).name + "): "
                    + loglikelihood_per_unit[i] + " (average: "
                    + loglikelihood_per_unit[i] / total_per_unit[i] + ")");
            }
        ...
        if (DEBUG) {
            System.out.println(
                "Final loglikelihood: " + loglikelihood + " (average " + (loglikelihood / total)
                    + ")");
        }
        //        return accuracy;
