...
public class GenerateTrainingTraces {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // LSI:
        bots.add(new LSI(MAX_PLAYOUTS, PLAYOUT_TIME, 0.75, LSI.EstimateType.RANDOM_TAIL,
            LSI.EstimateReuseType.ALL, LSI.GenerateType.PER_AGENT,
            Sampling.AgentOrderingType.ENTROPY, LSI.EvaluateType.HALVING, false,
            LSI.RelaxationType.NONE, 2, false, randomAI, ef));
        // NaiveMCTS up to depth 8:
        bots.add(new NaiveMCTS(TIME, MAX_PLAYOUTS, PLAYOUT_TIME, 16, 0.33f, 0.0f, 0.4f,
            new RandomBiasedAI(), new SimpleSqrtEvaluationFunction3(), true));

        String[] mapnames = {"maps/8x8/OneBaseWorker8x8.xml", "maps/8x8/TwoBasesWorkers8x8.xml",
            "maps/8x8/ThreeBasesWorkers8x8.xml", "maps/8x8/FourBasesWorkers8x8.xml",
            "maps/12x12/OneBaseWorker12x12.xml", "maps/12x12/TwoBasesWorkers12x12.xml",
            "maps/12x12/ThreeBasesWorkers12x12.xml", "maps/12x12/FourBasesWorkers12x12.xml",};
        PrintStream out = new PrintStream(
            new File("learningtracegeneration-" + MAX_PLAYOUTS + ".txt"));
        List<PhysicalGameState> maps = new LinkedList<>();
        for (String mapname : mapnames) {
            maps.add(PhysicalGameState.load(mapname, utt));
        }
        runExperiments(bots, maps, utt, EXPERIMENT_ITERATIONS, 3000, 300, false, out, -1, false,
            false, "learningtrace-" + MAX_PLAYOUTS);
    }
    ...
    public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps,
        ...
        boolean skip_self_play, boolean partiallyObservable, String tracePrefix) throws Exception {
            ...
            for (int ai2_idx = 0; ai2_idx < bots.size(); ai2_idx++) {
                ...
                for (PhysicalGameState pgs : maps) {
                    ...
                    for (int i = 0; i < iterations; i++) {
                        ...
                        TraceEntry te = new TraceEntry(gs.getPhysicalGameState().clone(),
                            gs.getTime());
                        trace.addEntry(te);
                        ...
                        if (visualize) {
                            w = PhysicalGameStatePanel
                                .newVisualizer(gs, 600, 600, partiallyObservable);
                        }
                        ...
                        String fileName =
                            tracePrefix + "-map" + maps.indexOf(pgs) + "-AI" + ai1_idx + "-AI"
                                + ai2_idx + "-" + i + ".xml";
