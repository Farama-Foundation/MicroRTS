...
public abstract class BayesianModel extends UnitActionProbabilityDistribution {
    ...
    public static List<UnitAction> generateAllPossibleUnitActions(UnitTypeTable utt) {
        ...
        for (UnitType ut : utt.getUnitTypes()) {
            if (ut.attackRange > maxAttackRange) {
    ...
    public abstract void train(List<int[]> x_l, List<Integer> y_l, List<TrainingInstance> i_l)
        throws Exception;
    ...
    public abstract void featureSelectionByCrossValidation(List<int[]> x_l, List<Integer> y_l,
        List<TrainingInstance> i_l) throws Exception;

    public abstract void featureSelectionByGainRatio(List<int[]> x_l, List<Integer> y_l,
    ...
    public double[] filterByPossibleActions(double[] d, Unit u, List<UnitAction> l) {
        ...
        for (int i = 0; i < l.size(); i++) {
            ...
            if (ua.getType() == UnitAction.TYPE_ATTACK_LOCATION) {
                ua = new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, ua.getLocationX() - u.getX(),
                    ua.getLocationY() - u.getY());
            }
    ...
    public double[] filterByPossibleActionIndexes(double[] predicted_distribution,
        List<Integer> possibleUnitActionIndexes) {
        double accum = 0;
