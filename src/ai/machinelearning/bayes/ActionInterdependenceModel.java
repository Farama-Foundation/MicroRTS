...
public class ActionInterdependenceModel extends BayesianModel {
    ...
    public ActionInterdependenceModel(int[] a_Xsizes, int a_Ysize, int estimation,
        double a_correctionFactor, UnitTypeTable utt, FeatureGenerator fg, String a_name) {
        super(utt, fg, a_name);
        Ysize = a_Ysize;
        Xsizes = a_Xsizes;
    ...
    public Object clone() {
        ActionInterdependenceModel c = new ActionInterdependenceModel(Xsizes, Ysize,
            estimationMethod, calibrationFactor, utt, featureGenerator, name);
        return c;
    }
    ...
    public void train(List<int[]> x_l, List<Integer> y_l, List<TrainingInstance> i_l)
        throws Exception {
        ...
        if (estimationMethod == ESTIMATION_COUNTS) {
            for (int i = 0; i < Ysize; i++) {
                prior_distribution[i] /= x_l.size();
            }
        } else {
            for (int i = 0; i < Ysize; i++) {
                prior_distribution[i] = (prior_distribution[i] + 1) / (x_l.size() + Ysize);
            }
        }
    }

    public void calibrateProbabilities(List<int[]> x_l, List<Integer> y_l,
        ...
        for (double c = 0.0; c <= 1.05; c += 0.05) {
            ...
            for (int i = 0; i < x_l.size(); i++) {
                ...
                for (UnitAction ua : possibleUnitActions) {
                    if (ua.getType() == UnitAction.TYPE_ATTACK_LOCATION) {
                        ua = new UnitAction(UnitAction.TYPE_ATTACK_LOCATION,
                            ua.getLocationX() - u.getX(), ua.getLocationY() - u.getY());
                    }
                ...
                if (possibleUnitActions.size() > 1) {
                    ...
                    predicted_distribution = filterByPossibleActionIndexes(predicted_distribution,
                        possibleUnitActionIndexes);
                    ...
                    int predicted_y = -1;
                    Collections.shuffle(
                        possibleUnitActions);   // shuffle it, just in case there are ties, to prevent action ordering bias
                    for (int idx : possibleUnitActionIndexes) {
                        if (predicted_y == -1) {
                            predicted_y = idx;
                        } else {
                            if (predicted_distribution[idx] > predicted_distribution[predicted_y]) {
                                predicted_y = idx;
                            }
                        }
                    }
        ...
        if (consider_action_types && selected_allowed_actiontype_prior != null) {
            ...
            for (int i : l) {
                ...
                for (int j : ltypes) {
                    if (j != i_type) {
                        if (estimationMethod == ESTIMATION_COUNTS) {
                            double p = selected_allowed_actiontype_prior[i_type][j]
                                / (double) actiontypes_allowed_counts_prior[i_type];
                            d[i] *= p;
                        } else {
                            double p =
                                (selected_allowed_actiontype_prior[i_type][j] + 1) / (double) (
                                    actiontypes_allowed_counts_prior[i_type] + 2);
                            d[i] *= p;
                        }
                    }
                }
        ...
        if (consider_individual_actions && selected_allowed_action_prior != null) {
            ...
            for (int i : l) {
                for (int j : l) {
                    if (j != i) {
                        if (estimationMethod == ESTIMATION_COUNTS) {
                            double p = selected_allowed_action_prior[i][j]
                                / (double) action_allowed_counts_prior[i];
                            d[i] *= p;
                        } else {
                            double p = (selected_allowed_action_prior[i][j] + 1) / (double) (
                                action_allowed_counts_prior[i] + 2);
                            d[i] *= p;
                        }
                    }
                }
    ...
    public void save(XMLWriter w) throws Exception {
        w.tagWithAttributes(getClass().getSimpleName(),
            "estimationMethod=\"" + estimationMethod + "\" " + "Ysize=\"" + Ysize + "\" "
                + "calibrationFactor=\"" + calibrationFactor + "\" " + "nfeatures=\""
                + distributions.length + "\" " + "featureGenerationClass=\"" + featureGenerator
        ...
        for (double v : prior_distribution) {
            w.rawXML(v + " ");
        }
        w.rawXML("\n");
        w.tag("/priorDistribution");
        if (selectedFeatures != null) {
            ...
            for (boolean v : selectedFeatures) {
                w.rawXML(v + " ");
            }
            w.rawXML("\n");
            w.tag("/selectedFeatures");
    ...
    public ActionInterdependenceModel(Element e, UnitTypeTable utt, String a_name)
        throws Exception {
    ...
    public void load(Element e) throws Exception {
        if (!e.getName().equals(getClass().getSimpleName())) {
            throw new Exception(
                "Head tag " + e.getName() + " is not '" + getClass().getSimpleName() + "'!");
        }
        // calculate the action types:
        allPossibleActionsTypes = new ArrayList<>();
        for (UnitAction ua : allPossibleActions) {
        ...
        Element xs_xml = e.getChild("Xsizes");
        {
            ...
            for (int i = 0; i < nfeatures; i++) {
                Xsizes[i] = Integer.parseInt(tokens[i]);
            }
        }
        Element pd_xml = e.getChild("priorDistribution");
        {
            ...
            prior_distribution = new double[Ysize];
            for (int i = 0; i < Ysize; i++) {
                prior_distribution[i] = Double.parseDouble(tokens[i]);
            }
        }
        ...
        if (sf_xml != null) {
            ...
            selectedFeatures = new boolean[nfeatures];
            for (int i = 0; i < nfeatures; i++) {
                selectedFeatures[i] = Boolean.parseBoolean(tokens[i]);
            }
        } else {
        ...
        }
        {
            ...
            action_allowed_counts_prior = new int[Ysize];
            for (int i = 0; i < Ysize; i++) {
                action_allowed_counts_prior[i] = Integer.parseInt(tokens[i]);
            }
        }
        {
        ...
        {
            Element actiontypes_allowed_counts_prior_xml = e
                .getChild("actiontypes_allowed_counts_prior");
            String text = actiontypes_allowed_counts_prior_xml.getTextTrim();
            String[] tokens = text.split(" ");
            actiontypes_allowed_counts_prior = new int[YtypeSize];
            for (int i = 0; i < YtypeSize; i++) {
                actiontypes_allowed_counts_prior[i] = Integer.parseInt(tokens[i]);
            }
        }
        {
            Element selected_allowed_actiontype_prior_xml = e
                .getChild("selected_allowed_actiontype_prior");
