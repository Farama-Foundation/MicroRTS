...
public class DiscreteCPD {
    ...
    public double[] marginalizedDistribution() {
        ...
        for (int i = 0; i < Yvalues; i++) {
            distribution[i] = ((double) marginalizedCounts[i]) / accum;
        }
    ...
    public double[] marginalizedDistributionLaplace(double beta) {
        ...
        for (int i = 0; i < Yvalues; i++) {
            distribution[i] = ((double) marginalizedCounts[i] + beta) / (accum + beta * Yvalues);
        }
    ...
    public double[] distribution(int Xvalue) {
        ...
        int accum = 0;
        for (int i = 0; i < Yvalues; i++) {
            accum += counts[Xvalue][i];
        }
        for (int i = 0; i < Yvalues; i++) {
            distribution[i] = ((double) counts[Xvalue][i]) / accum;
        }
    ...
    public double[] distributionLaplace(int Xvalue, double beta) {
        ...
        for (int i = 0; i < Yvalues; i++) {
            distribution[i] = ((double) counts[Xvalue][i] + beta) / (accum + beta * Yvalues);
        }
    ...
    public void save(XMLWriter w) throws Exception {
        w.tagWithAttributes("DiscreteCPD",
