...
public class TwoPhaseNaiveMCTSNode extends NaiveMCTSNode {
    ...
    public TwoPhaseNaiveMCTSNode(int maxplayer, int minplayer, GameState a_gs,
        NaiveMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, boolean fensa)
        throws Exception {
        super(maxplayer, minplayer, a_gs, a_parent, a_evaluation_bound, a_creation_ID, fensa);
    }

    // Naive Sampling:
    public TwoPhaseNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float el1, float eg1,
        float e01, int a_gs1, float el2, float eg2, float e02, int a_gs2, int phase1_budget,
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            if (global_strategy == E_GREEDY) {
                selected = (TwoPhaseNaiveMCTSNode) selectFromAlreadySampledEpsilonGreedy(epsilon_g);
            } else if (global_strategy == UCB1) {
                selected = (TwoPhaseNaiveMCTSNode) selectFromAlreadySampledUCB1(C);
            }
            return selected
                .selectLeaf(maxplayer, minplayer, el1, eg1, e01, a_gs1, el2, eg2, e02, a_gs2,
                    phase1_budget, max_depth, a_creation_ID);
        } else {
            // sample from the local MABs (this might recursively call "selectLeaf" internally):
            return selectLeafUsingLocalMABs(maxplayer, minplayer, el1, eg1, e01, a_gs1, el2, eg2,
                e02, a_gs2, phase1_budget, max_depth, a_creation_ID);
        }
    }

    public TwoPhaseNaiveMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float el1,
        float eg1, float e01, int a_gs1, float el2, float eg2, float e02, int a_gs2,
        int phase1_budget, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        if (pate == null) {
            ...
            TwoPhaseNaiveMCTSNode node = new TwoPhaseNaiveMCTSNode(maxplayer, minplayer,
                gs2.clone(), this, evaluation_bound, a_creation_ID,
        ...
        return pate.selectLeaf(maxplayer, minplayer, el1, eg1, e01, a_gs1, el2, eg2, e02, a_gs2,
            phase1_budget, max_depth, a_creation_ID);
    }
}