...
public class TwoPhaseNaiveMCTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public TwoPhaseNaiveMCTS(UnitTypeTable utt) {
        this(100, -1, 100, 10, 0.3f, 0.0f, 1.0f, 0.3f, 0.0f, 0.0f, 0.5f, new RandomBiasedAI(),
    ...
    public TwoPhaseNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
        float el1, float eg1, float e01, float el2, float eg2, float e02, float p1_ratio, AI policy,
    ...
    public TwoPhaseNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
        float el1, float eg1, float e01, int a_gs1, float el2, float eg2, float e02, int a_gs2,
        float p1_ratio, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        randomAI = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public AI clone() {
        return new TwoPhaseNaiveMCTS(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME,
            MAX_TREE_DEPTH, phase1_epsilon_l, phase1_epsilon_g, phase1_epsilon_0, phase2_epsilon_l,
            phase2_epsilon_g, phase2_epsilon_0, phase1_ratio, randomAI, ef,
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        ...
        node_creation_ID = 0;
        tree = new NaiveMCTSNode(playerForThisComputation, 1 - playerForThisComputation, gs, null,
            ef.upperBound(gs), node_creation_ID++, forceExplorationOfNonSampledActions);
        ...
        if (ITERATIONS_BUDGET > 0) {
            n_phase1_iterations_left = (int) (phase1_ratio * ITERATIONS_BUDGET);
        }
        if (TIME_BUDGET > 0) {
            n_phase1_milliseconds_left = (int) (phase1_ratio * TIME_BUDGET);
        }
    }
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        while (true) {
            if (n_phase1_milliseconds_left > 0) {
                n_phase1_milliseconds_left =
                    n_phase1_milliseconds_left_initial - (int) (end - start);
            }
            if (!iteration(playerForThisComputation)) {
                break;
            }
            count++;
    ...
    public boolean iteration(int player) throws Exception {
        ...
        if (n_phase1_iterations_left > 0 || n_phase1_milliseconds_left > 0) {
            leaf = tree.selectLeaf(player, 1 - player, phase1_epsilon_l, phase1_epsilon_g,
                phase1_epsilon_0, phase1_global_strategy, MAX_TREE_DEPTH, node_creation_ID++);
            n_phase1_iterations_left--;
        } else {
            leaf = tree.selectLeaf(player, 1 - player, phase2_epsilon_l, phase2_epsilon_g,
                phase2_epsilon_0, phase2_global_strategy, MAX_TREE_DEPTH, node_creation_ID++);
        }
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (idx == -1) {
            if (DEBUG >= 1) {
                System.out
                    .println("TwoPhaseNaiveMCTS no children selected. Returning an empty asction");
            }
            return new PlayerAction();
        }
        if (DEBUG >= 2) {
        ...
        if (DEBUG >= 1) {
            ...
            System.out.println(
                "TwoPhaseNaiveMCTS selected children " + tree.actions.get(idx) + " explored "
    ...
    public String toString() {
        return getClass().getSimpleName() + "(" + TIME_BUDGET + ", " + ITERATIONS_BUDGET + ", "
            + MAXSIMULATIONTIME + "," + MAX_TREE_DEPTH + "," + phase1_epsilon_l + ", "
            + phase1_epsilon_g + ", " + phase1_epsilon_0 + ", " + phase2_epsilon_l + ", "
            + phase2_epsilon_g + ", " + phase2_epsilon_0 + ", " + phase1_ratio + ", " + randomAI
            + ", " + ef + ")";
    }
    ...
    public String statisticsString() {
        return "Total runs: " + total_runs + ", runs per action: " + (total_runs
            ...
            / (float) total_cycles_executed) + ", averate time per cycle: " + (total_time
    ...
    public List<ParameterSpecification> getParameters() {
        ...
        ParameterSpecification ps_ratio = new ParameterSpecification("Phase1_Ratio", float.class,
            0.5);
        ...
        parameters.add(
            new ParameterSpecification("ForceExplorationOfNonSampledActions", boolean.class, true));
