...
public class TwoPhaseNaiveMCTSPerNode extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public TwoPhaseNaiveMCTSPerNode(UnitTypeTable utt) {
        this(100, -1, 100, 10, 0.3f, 0.0f, 1.0f, 0.3f, 0.0f, 0.0f, 100, new RandomBiasedAI(),
            new SimpleSqrtEvaluationFunction3(), true);
    }

    public TwoPhaseNaiveMCTSPerNode(int available_time, int max_playouts, int lookahead,
        int max_depth, float el1, float eg1, float e01, float el2, float eg2, float e02,
        int p1_budget, AI policy, EvaluationFunction a_ef, boolean fensa) {
    ...
    public TwoPhaseNaiveMCTSPerNode(int available_time, int max_playouts, int lookahead,
        int max_depth, float el1, float eg1, float e01, int a_gs1, float el2, float eg2, float e02,
        int a_gs2, int p1_budget, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        randomAI = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public AI clone() {
        return new TwoPhaseNaiveMCTSPerNode(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME,
            MAX_TREE_DEPTH, phase1_epsilon_l, phase1_epsilon_g, phase1_epsilon_0, phase2_epsilon_l,
            phase2_epsilon_g, phase2_epsilon_0, phase1_budget, randomAI, ef,
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        ...
        node_creation_ID = 0;
        tree = new TwoPhaseNaiveMCTSNode(playerForThisComputation, 1 - playerForThisComputation, gs,
            null, ef.upperBound(gs), node_creation_ID++, forceExplorationOfNonSampledActions);
    ...
    public boolean iteration(int player) throws Exception {
        ...
        leaf = tree
            .selectLeaf(player, 1 - player, phase1_epsilon_l, phase1_epsilon_g, phase1_epsilon_0,
                phase1_global_strategy, phase2_epsilon_l, phase2_epsilon_g, phase2_epsilon_0,
                phase2_global_strategy, phase1_budget, MAX_TREE_DEPTH, node_creation_ID++);
    ...
    public String toString() {
        return getClass().getSimpleName() + "(" + TIME_BUDGET + ", " + ITERATIONS_BUDGET + ", "
            ...
            + phase2_epsilon_g + ", " + phase2_epsilon_0 + ", " + phase1_budget + ", " + randomAI
            + ", " + ef + ")";
    }
