...
public class DownsamplingUCTNode {
    ...
    public DownsamplingUCTNode(int maxplayer, int minplayer, GameState a_gs,
        DownsamplingUCTNode a_parent, long MAXACTIONS, float bound) throws Exception {
        parent = a_parent;
        gs = a_gs;
        if (parent == null) {
            depth = 0;
    ...
    public DownsamplingUCTNode UCTSelectLeaf(int maxplayer, int minplayer, long MAXACTIONS,
        long cutOffTime, int max_depth) throws Exception {
        ...
        if (hasMoreActions) {
            ...
            if (children.size() >= actions.size()) {
                ...
                DownsamplingUCTNode node = new DownsamplingUCTNode(maxplayer, minplayer,
                    gs2.clone(), this, MAXACTIONS, evaluation_bound);
                children.add(node);
                return node;
            }
        }
    ...
    public void showNode(int depth, int maxdepth) {
        if (children != null) {
            for (int i = 0; i < children.size(); i++) {
                ...
                System.out.println("child " + actions.get(i) + " explored " + child.visit_count
