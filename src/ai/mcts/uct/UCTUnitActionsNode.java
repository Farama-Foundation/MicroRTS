...
public class UCTUnitActionsNode {
    ...
    public UCTUnitActionsNode(int maxplayer, int minplayer, GameState a_gs,
        UCTUnitActionsNode a_parent, float bound) {
        parent = a_parent;
        if (parent == null) {
            depth = 0;
        } else {
    ...
    public UCTUnitActionsNode UCTSelectLeaf(int maxplayer, int minplayer, int max_depth) {
        ...
        if (children.size() < actions.size()) {
            ...
            if (a != null) {
                ...
                UCTUnitActionsNode node = new UCTUnitActionsNode(maxplayer, minplayer, gs2.clone(),
                    this, evaluation_bound);
                children.add(node);
                return node;
            }
        }
    ...
    public void showNode(int depth, int maxdepth) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            System.out.println(
                "child " + actions.get(i) + " explored " + child.visit_count + " Avg evaluation: "
                    + (child.accum_evaluation / ((double) child.visit_count)));
            if (depth < maxdepth) {
