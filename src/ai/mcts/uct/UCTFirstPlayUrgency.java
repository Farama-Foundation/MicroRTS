...
public class UCTFirstPlayUrgency extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public UCTFirstPlayUrgency(int available_time, int max_playouts, int lookahead, int max_depth,
        AI policy, EvaluationFunction a_ef, double a_FPUvalue) {
    ...
    public AI clone() {
        return new UCTFirstPlayUrgency(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME,
            MAX_TREE_DEPTH, randomAI, ef, FPUvalue);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        ...
        tree = new UCTNodeFirstPlayUrgency(playerForThisComputation, 1 - playerForThisComputation,
            gs, null, evaluation_bound, FPUvalue);
        gs_to_start_from = gs;
        total_runs_this_move = 0;
    ...
    public double monteCarloRun(int player, long cutOffTime) throws Exception {
        UCTNodeFirstPlayUrgency leaf = tree
            .UCTSelectLeaf(player, 1 - player, cutOffTime, MAX_TREE_DEPTH);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (tree.children == null) {
            if (DEBUG >= 1) {
                System.out.println(this.getClass().getSimpleName()
                    + " no children selected. Returning an empty asction");
            }
            return new PlayerAction();
        }
        ...
        if (DEBUG >= 1) {
            System.out.println(
                this.getClass().getSimpleName() + " performed " + total_runs_this_move
                    + " playouts.");
        }
        if (DEBUG >= 1) {
            System.out.println(
