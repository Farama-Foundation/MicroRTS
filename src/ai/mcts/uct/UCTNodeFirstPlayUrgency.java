...
public class UCTNodeFirstPlayUrgency {
    ...
    public UCTNodeFirstPlayUrgency(int maxplayer, int minplayer, GameState a_gs,
        UCTNodeFirstPlayUrgency a_parent, float bound, double a_FPUValue) throws Exception {
        parent = a_parent;
        gs = a_gs;
        if (parent == null) {
            depth = 0;
    ...
    public UCTNodeFirstPlayUrgency UCTSelectLeaf(int maxplayer, int minplayer, long cutOffTime,
        ...
        // First Play Urgency:
        if (best != null && best_score > FPUvalue) {
        ...
        if (moveGenerator != null) {
            ...
            if (attemptsLeft > 0) {
                ...
                UCTNodeFirstPlayUrgency node = new UCTNodeFirstPlayUrgency(maxplayer, minplayer,
                    gs2.clone(), this, evaluation_bound, FPUvalue);
                children.add(node);
    ...
    public double childValue(UCTNodeFirstPlayUrgency child) {
        ...
        double exploration = Math.sqrt(Math.log(visit_count) / child.visit_count);
        if (type == 0) {
            // max node:
    ...
    public void showNode(int depth, int maxdepth) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            System.out.println("child explored " + child.visit_count + " Avg evaluation: " + (
                child.accum_evaluation / ((double) child.visit_count)) + " : " + actions.get(i));
            if (depth < maxdepth) {
