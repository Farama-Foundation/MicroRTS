...
public class DownsamplingUCT extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public DownsamplingUCT(UnitTypeTable utt) {
        this(100, -1, 100, 100, 10, new RandomBiasedAI(), new SimpleSqrtEvaluationFunction3());
    }

    public DownsamplingUCT(int available_time, int max_playouts, int lookahead, long maxactions,
        int max_depth, AI policy, EvaluationFunction a_ef) {
    ...
    public AI clone() {
        return new DownsamplingUCT(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAXACTIONS,
            MAX_TREE_DEPTH, randomAI, ef);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        ...
        tree = new DownsamplingUCTNode(playerForThisComputation, 1 - playerForThisComputation, gs,
            null, MAXACTIONS, evaluation_bound);
        gs_to_start_from = gs;
    }
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        while (true) {
            DownsamplingUCTNode leaf = tree
                .UCTSelectLeaf(playerForThisComputation, 1 - playerForThisComputation, MAXACTIONS,
                    cutOffTime, MAX_TREE_DEPTH);
            ...
            if (leaf != null) {
                ...
                double evaluation =
                    ef.evaluate(playerForThisComputation, 1 - playerForThisComputation, gs2) * Math
                        .pow(0.99, time / 10.0);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (mostVisitedIdx == -1) {
            System.err.println(
                "DownsamplingUCT.getBestActionSoFar: mostVisitedIdx == -1!!! tree.children.size() = "
                    + tree.children.size());
        ...
        if (DEBUG >= 1) {
            System.out.println(
                this.getClass().getSimpleName() + " selected children " + tree.actions
                    .get(mostVisitedIdx) + " explored " + mostVisited.visit_count
                    + " Avg evaluation: " + (mostVisited.accum_evaluation
                    / ((double) mostVisited.visit_count)));
        }
    ...
    public String toString() {
        return getClass().getSimpleName() + "(" + TIME_BUDGET + ", " + ITERATIONS_BUDGET + ", "
            + MAXSIMULATIONTIME + ", " + MAXACTIONS + ", " + MAX_TREE_DEPTH + ", " + randomAI + ", "
            + ef + ")";
    }
