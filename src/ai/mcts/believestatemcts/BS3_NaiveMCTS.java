...
/**
 * The believe state stores the last observed location of an opponent's unit. If the locations is
 * visible again we search for the nearest not visible location. We do inference on: - possible
 * buildings not seen before (i.e. if we see a military unit, it should be a barracks) - when a new
 * visible unit, remove closest last seen position
 *
 * @author albertouri
 */
public class BS3_NaiveMCTS extends NaiveMCTS implements AIWithBelieveState {
    ...
    public BS3_NaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
        float e_l, float discout_l, float e_g, float discout_g, float e_0, float discout_0,
        AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts, lookahead, max_depth, e_l, discout_l, e_g, discout_g,
            e_0, discout_0, policy, a_ef, fensa);
    }

    public BS3_NaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
        float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts, lookahead, max_depth, e_l, e_g, e_0, policy, a_ef,
            fensa);
    }

    public BS3_NaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
        float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef,
        boolean fensa) {
        super(available_time, max_playouts, lookahead, max_depth, e_l, e_g, e_0, a_global_strategy,
            policy, a_ef, fensa);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        if (initialGameState != null && gs.getTime() == 0) {
            setInitialBelieveState(a_player, initialGameState.clone(),
                new PartiallyObservableGameState(initialGameState, a_player));
        }
        ...
        player = a_player;
        current_iteration = 0;
        tree = new NaiveMCTSNode(player, 1 - player, gs, null, ef.upperBound(gs),
    ...
    public void setInitialBelieveState(int player, GameState gs,
        ...
        typeSeen = new boolean[gs.getUnitTypeTable().getUnitTypes()
            .size()]; // default value of false
        ...
        for (Unit u : lastObservedGame.getUnits()) {
            if (u.getPlayer() == opponent && gs.free(u.getX(), u.getY())) {
                ...
                if (uaa != null && uaa.action.getType() == UnitAction.TYPE_MOVE) {
                    int offsx = 0;
                    int offsy = 0;
                    if (uaa.action.getDirection() == UnitAction.DIRECTION_UP) {
                        offsy = -1;
                    }
                    if (uaa.action.getDirection() == UnitAction.DIRECTION_RIGHT) {
                        offsx = 1;
                    }
                    if (uaa.action.getDirection() == UnitAction.DIRECTION_DOWN) {
                        offsy = 1;
                    }
                    if (uaa.action.getDirection() == UnitAction.DIRECTION_LEFT) {
                        offsx = -1;
                    }
                    ...
                    if (!gs.observable(u.getX(), u.getY())) { // is location still observable?
                        if (!wasUnderAttack(
                            u)) { // wasn't under attack (sometimes units kill each other simultaneously)
            ...
            if (u.getPlayer() == opponent) {
                ...
                if (!typeSeen[u.getType().ID]) {
                    ...
                    if (!typeSeen[ut.ID]) {
                        ...
                        boolean validPosition = getClosestNotObservableLocationNear(u.getX(),
                            u.getY(), gs, newUnit);
                        if (validPosition) {
                ...
                if (!lastObservedGame.observable(u.getX(), u.getY()) || !wasVisibleOpponentMovingTo(
                    opponent, u.getX(), u.getY())) {
                ...
                if (uaa != null && (uaa.action.getType() == UnitAction.TYPE_MOVE
                    ...
                    for (Unit bu : lastKnownPosition) {
                        if (bu.getX() == offsx && bu.getY() == offsy) {
                            ...
                            boolean validPosition = getClosestNotObservableLocationNear(bu.getX(),
                                bu.getY(), gs, bu);
                            if (!validPosition) {
    ...
    public boolean wasVisibleOpponentMovingTo(int opponent, int x, int y) {
        if (!lastObservedGame.free(x, y)) {
            return true; // unit was already there
        }
        for (Unit u : lastObservedGame.getUnits()) {
            if (u.getPlayer() == opponent) {
                UnitActionAssignment uaa = lastObservedGame.getActionAssignment(u);
                if (uaa != null && uaa.action.getType() == UnitAction.TYPE_MOVE) {
                    ...
                    if ((u.getX() + offsx) == x && (u.getY() + offsy) == y) {
                        return true;
                    }
                }
            }
    ...
    public boolean getClosestNotObservableLocationNear(int startX, int startY,
        PartiallyObservableGameState gs, Unit u) {
        ...
        int dy = 1;
        int maxLenght = Math
            .max(gs.getPhysicalGameState().getWidth(), gs.getPhysicalGameState().getHeight());
        while (length < maxLenght) {
            // look for a location that is not visible and free
            if (x >= 0 && x < gs.getPhysicalGameState().getWidth() && y >= 0 && y < gs
                .getPhysicalGameState().getHeight() && !gs.observable(x, y) && gs.free(x, y)
                && believeFree(x, y)) {
                u.setX(x);
                u.setY(y);
                return true;
