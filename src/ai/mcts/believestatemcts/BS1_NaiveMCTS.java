...
/**
 * The believe state only stores the opponent building from the game state at frame 0. There isn't
 * new observations, and the buildings in the believe state are only removed when the building is
 * destroyed.
 *
 * @author albertouri
 */
public class BS1_NaiveMCTS extends NaiveMCTS implements AIWithBelieveState {
    ...
    public void setInitialBelieveState(int player, GameState gs,
        ...
        for (Unit u : gs.getUnits()) {
            if (u.getPlayer() == opponent && !u.getType().canMove && !pogs
                .observable(u.getX(), u.getY())) {
