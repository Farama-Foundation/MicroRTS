 ...
 * @author santi
 * <p>
 * From: "Learning Multiuser Channel Allocations in Cognitive Radio Networks: A Combinatorial
 * Multi-Armed Bandit Formulation" Yi Gai, Bhaskar Krishnamachari and Rahul Jain
 * <p>
 * The original NLPS sampling strategy used the Hungarian algorithm to selct the best macro-arm at
 * each cycle. However, that only works in their formulation. In the more general case that we
 * consider here, the Hungarian algorithm is not applicable. Thus, I replaced that step by simply
 * selecting the actino with the maximum score for each unit. If the actions do not interfere too
 * much, this obtains the obtimal action most of the times, and it's O(n*m)
 */
public class MLPSNode extends MCTSNode {
    ...
    public MLPSNode(int maxplayer, int minplayer, GameState a_gs, MLPSNode a_parent, double bound,
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            for (Pair<Unit, List<UnitAction>> choice : moveGenerator.getChoices()) {
                ...
                if (ae.nactions > max_nactions) {
                    max_nactions = ae.nactions;
                }
    ...
    public double explorationValue(int M, int n, int n_ij) {
        ...
        double exploration = M * Math.sqrt((M + 1) * Math.log(n) / n_ij);
        return exploration;
    }

    // C is the UCB constant for exploration/exploitation
    public MLPSNode selectLeaf(int maxplayer, int minplayer, double C, int max_depth,
        int a_creation_ID) throws Exception {
        ...
        for (int ate_idx = 0; ate_idx < unitActionTable.size(); ate_idx++) {
            ...
            for (int i = 0; i < ate.nactions; i++) {
                ...
                scoresExploration[i] = explorationValue(max_nactions, visit_count,
                    ate.visit_count[i]);
            }
            ...
            if (DEBUG >= 3) {
                System.out.print("[ ");
                for (int i = 0; i < ate.nactions; i++) {
                    System.out.print("(" + ate.visit_count[i] + "," + scoresExploitation[i] + ","
                        + scoresExploration[i] + ")");
                }
                System.out.println("]");
            }
        ...
        for (int repeat = 0; repeat < 10; repeat++) {
            ...
            List<Integer> notSampledYetIDs2 = new LinkedList<Integer>(notSampledYetIDs);
            while (!notSampledYetIDs2.isEmpty()) {
                ...
                try {
                    ...
                    for (int j = 0; j < ate.nactions; j++) {
                        ...
                        double s1 = scoresExploitation[j] + C * Math
                            .max(scoresExploration[j], maxExplorationScore);
                        double s2 = scoresExploitation[code] + C * Math
                            .max(scoresExploration[code], maxExplorationScore);
                        if (s1 > s2) {
                            code = j;
                        }
                    }
        ...
        if (pate == null) {
            ...
            MLPSNode node = new MLPSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound,
                a_creation_ID);
