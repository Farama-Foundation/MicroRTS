...
public class InformedNaiveMCTSNode extends MCTSNode {
    ...
    public InformedNaiveMCTSNode(int maxplayer, int minplayer, GameState a_gs,
        UnitActionProbabilityDistribution a_bias, InformedNaiveMCTSNode a_parent,
        double a_evaluation_bound, int a_creation_ID) throws Exception {
        parent = a_parent;
        gs = a_gs;
        model = a_bias;
        if (parent == null) {
        ...
        while (gs.winner() == -1 && !gs.gameover() && !gs.canExecuteAnyAction(maxplayer) && !gs
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            for (Pair<Unit, List<UnitAction>> choice : moveGenerator.getChoices()) {
                ...
                InformedUnitActionTableEntry ae = new InformedUnitActionTableEntry(choice.m_a,
                    choice.m_b, prior_distribution);
    ...
    // Naive Sampling:
    public InformedNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l,
        float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID)
        throws Exception {
        ...
        */
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            return selected
                .selectLeaf(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0, global_strategy,
                    max_depth, a_creation_ID);
        } else {
            // sample from the local MABs (this might recursively call "selectLeaf" internally):
            return selectLeafUsingLocalMABs(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0,
                global_strategy, max_depth, a_creation_ID);
        }
    }

    public InformedNaiveMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g)
        throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            for (MCTSNode pate : children) {
                if (type == 0) {
                    // max node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) > (
                        best.accum_evaluation / best.visit_count)) {
            ...
            InformedNaiveMCTSNode best = (InformedNaiveMCTSNode) children
                .get(r.nextInt(children.size()));
            return best;
        }
    }
    ...
    public InformedNaiveMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer,
        float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth,
        ...
        for (InformedUnitActionTableEntry ate : unitActionTable) {
            ...
            for (int i = 0; i < ate.nactions; i++) {
                if (type == 0) {
                    // max node:
                    if (bestIdx == -1 || (visits != 0 && ate.visit_count[i] == 0) || (visits != 0
                        && (ate.accum_evaluation[i] / ate.visit_count[i]) > bestEvaluation)) {
                        bestIdx = i;
            ...
            if (DEBUG >= 3) {
                ...
                System.out.print("[ ");
                for (int i = 0; i < ate.nactions; i++) {
                    System.out.print("(" + ate.visit_count[i] + ","
                        + ate.accum_evaluation[i] / ate.visit_count[i] + ")");
                }
                System.out.println("]");
        ...
        if (pate == null) {
            ...
            InformedNaiveMCTSNode node = new InformedNaiveMCTSNode(maxplayer, minplayer,
                gs2.clone(), model, this, evaluation_bound, a_creation_ID);
