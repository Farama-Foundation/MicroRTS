...
public class MonteCarlo extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public MonteCarlo(int available_time, int playouts_per_cycle, int lookahead, AI policy,
    ...
    public MonteCarlo(int available_time, int playouts_per_cycle, int lookahead, long maxactions,
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        if (actions == null) {
            ...
            if (MAXACTIONS > 0 && moveGenerator.getSize() > 2 * MAXACTIONS) {
                ...
                if (DEBUG >= 1) {
                    System.out.println("MontCarloAI (random action sampling) for player "
                        + playerForThisComputation + " chooses between " + moveGenerator.getSize()
                        + " actions [maximum so far " + max_actions_so_far + "] (cycle "
                        + gs_to_start_from.getTime() + ")");
                }
            } else {
                PlayerAction pa;
                long count = 0;
                do {
                    ...
                    if (pa != null) {
                        ...
                        actions.add(pate);
                        count++;
                        if (MAXACTIONS > 0 && count >= 2 * MAXACTIONS) {
                ...
                if (DEBUG >= 1) {
                    System.out.println(
                        "MontCarloAI (complete generation plus random reduction) for player "
                            + playerForThisComputation + " chooses between " + actions.size()
                            + " actions [maximum so far " + max_actions_so_far + "] (cycle "
                            + gs_to_start_from.getTime() + ")");
                }
                while (MAXACTIONS > 0 && actions.size() > MAXACTIONS) {
                    actions.remove(r.nextInt(actions.size()));
                }
            }
        }

        while (true) {
            if (TIME_BUDGET > 0 && (System.currentTimeMillis() - start) >= TIME_BUDGET) {
                break;
            }
            if (ITERATIONS_BUDGET > 0 && nruns >= ITERATIONS_BUDGET) {
                break;
            }
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        for (PlayerActionTableEntry pate : actions) {
            if (best == null || (pate.accum_evaluation / pate.visit_count) > (best.accum_evaluation
                / best.visit_count)) {
                best = pate;
            }
        }
        if (best == null) {
            ...
            pate.pa = moveGenerator.getRandom();
            System.err.println(
                "MonteCarlo.getBestActionSoFar: best action was null!!! action.size() = " + actions
                    .size());
        }

        if (DEBUG >= 1) {
            System.out.println("Executed " + run + " runs");
            System.out.println("Selected action: " + best + " visited " + best.visit_count
                + " with average evaluation " + (best.accum_evaluation / best.visit_count));
        }
