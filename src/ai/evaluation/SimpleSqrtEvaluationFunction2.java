 ...
 * @author santi
 * <p>
 * This function is similar to SimpleSqrtEvaluationFunction, except that it detects when a player
 * has won and returns a special, larger value.
 */
public class SimpleSqrtEvaluationFunction2 extends EvaluationFunction {
    ...
    public float base_score(int player, GameState gs) {
        ...
        for (Unit u : pgs.getUnits()) {
            if (u.getPlayer() == player) {
                ...
                score += UNIT_BONUS_MULTIPLIER * (u.getCost() * Math.sqrt(u.getHitPoints()) / u
                    .getMaxHitPoints());
            }
        }
        if (!anyunit) {
    ...
    public float upperBound(GameState gs) {
        ...
        return (free_resources + Math.max(player_resources[0], player_resources[1]))
            * UNIT_BONUS_MULTIPLIER;
    }
}
