 ...
 * memory allocation penalty. It only reallocates memory when asked to path-find for first time, or
 * in a map that is bigger than the previous time.
 */
        ...
        while (openinsert > 0) {
            ...
            */
            ...
            if (y > 0 && inOpenOrClosed[pos - w] == 0) {
                if (free[x][y - 1] == null) {
                    free[x][y - 1] = gs.free(x, y - 1);
                }
                assert (free[x][y - 1] != null);
                if (free[x][y - 1]) {
                    addToOpen(x, y - 1, pos - w, pos,
                        manhattanDistance(x, y - 1, targetx, targety));
                }
            }
            if (x < pgs.getWidth() - 1 && inOpenOrClosed[pos + 1] == 0) {
                if (free[x + 1][y] == null) {
                    free[x + 1][y] = gs.free(x + 1, y);
                }
                assert (free[x + 1][y] != null);
                if (free[x + 1][y]) {
                    addToOpen(x + 1, y, pos + 1, pos,
                        manhattanDistance(x + 1, y, targetx, targety));
                }
            }
