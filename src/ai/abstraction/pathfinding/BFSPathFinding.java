    ...
    public UnitAction findPath(Unit start, int targetpos, GameState gs, ResourceUsage ru) {        
        ...
        while(openinsert!=openremove) {
            ...
            if (pos == targetpos) {
                ...
                int last = pos;
//                System.out.println("- Path from " + start.getX() + "," + start.getY() + " to " + targetpos%w + "," + targetpos/w + " (range " + 0 + ")");
                while(parent!=pos) {
                    last = pos;
                    pos = parent;
                    parent = closed[pos];
            ...
            if (y>0 && inOpenOrClosed[pos-w] == 0 && free[x][y-1]) {
            ...
            if (x<pgs.getWidth()-1 && inOpenOrClosed[pos+1] == 0 && free[x+1][y]) {
            ...
            if (y<pgs.getHeight()-1 && inOpenOrClosed[pos+w] == 0 && free[x][y+1]) {
                open[openinsert] = (pos+w);
                parents[openinsert] = (pos);
                openinsert++;
                if (openinsert>=open.length) openinsert = 0;
            ...
            if (x>0 && inOpenOrClosed[pos-1] == 0 && free[x-1][y]) {
                open[openinsert] = (pos-1);
                parents[openinsert] = (pos);
                openinsert++;
                if (openinsert>=open.length) openinsert = 0;
