...
public class FloodFillPathFinding extends PathFinding {
    ...
    public UnitAction findPathToPositionInRange(Unit start, int targetpos, int range, GameState gs,
        ...
        if ((start.getX() - x) * (start.getX() - x) + (start.getY() - y) * (start.getY() - y)
            <= range * range) {//already there
    ...
    private UnitAction calculateDistances(Unit start, int targetpos, int range, GameState gs,
        ResourceUsage ru) {
        int x = targetpos % w;
        int y = targetpos / w;
    ...
    private void doFloodFill(int x, int y, GameState gs, int finalX, int finalY) {
        ...
        while (index < fringe.size()) {
            ...
            if (bounds(nextX, nextY) && distances[nextX][nextY] == Integer.MAX_VALUE
                && free[nextX][nextY] && gsFree[nextX][nextY]) {
