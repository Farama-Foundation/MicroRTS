...
public class Build extends AbstractAction {
    ...
    public void toxml(XMLWriter w) {
        w.tagWithAttributes("Build",
            "unitID=\"" + unit.getID() + "\" type=\"" + type.name + "\" x=\"" + x + "\" y=\"" + y
                + "\" pathfinding=\"" + pf.getClass().getSimpleName() + "\"");
        w.tag("/Build");
    }
    ...
    public UnitAction execute(GameState gs, ResourceUsage ru) {
        ...
        if (move != null) {
            if (gs.isUnitActionAllowed(unit, move)) {
                return move;
            }
            return null;
        }
        ...
        // build:
        UnitAction ua = null;
        if (x == unit.getX() && y == unit.getY() - 1) {
            ua = new UnitAction(UnitAction.TYPE_PRODUCE, UnitAction.DIRECTION_UP, type);
        }
        if (x == unit.getX() + 1 && y == unit.getY()) {
            ua = new UnitAction(UnitAction.TYPE_PRODUCE, UnitAction.DIRECTION_RIGHT, type);
        }
        if (x == unit.getX() && y == unit.getY() + 1) {
            ua = new UnitAction(UnitAction.TYPE_PRODUCE, UnitAction.DIRECTION_DOWN, type);
        }
        if (x == unit.getX() - 1 && y == unit.getY()) {
            ua = new UnitAction(UnitAction.TYPE_PRODUCE, UnitAction.DIRECTION_LEFT, type);
        }
