    ...
    public void toxml(XMLWriter w) {
        w.tagWithAttributes("Harvest",
            "unitID=\"" + unit.getID() + "\" target=\"" + target.getID() + "\" base=\"" + base
                .getID() + "\" pathfinding=\"" + pf.getClass().getSimpleName() + "\"");
        w.tag("/Harvest");
    }
    ...
    public UnitAction execute(GameState gs, ResourceUsage ru) {
        ...
        if (unit.getResources() == 0) {
            ...
            UnitAction move = pf.findPathToAdjacentPosition(unit,
                target.getX() + target.getY() * gs.getPhysicalGameState().getWidth(), gs, ru);
            if (move != null) {
            ...
            if (target.getX() == unit.getX() + 1 && target.getY() == unit.getY()) {
            ...
            UnitAction move = pf.findPathToAdjacentPosition(unit,
                base.getX() + base.getY() * gs.getPhysicalGameState().getWidth(), gs, ru);
            if (move != null) {
                if (gs.isUnitActionAllowed(unit, move)) {
                    return move;
                }
            ...
            if (base.getX() == unit.getX() + 1 && base.getY() == unit.getY()) {
