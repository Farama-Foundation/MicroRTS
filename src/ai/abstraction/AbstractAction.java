...
public abstract class AbstractAction {
    ...
    public static AbstractAction fromXML(Element e, PhysicalGameState gs, UnitTypeTable utt) {
        ...
        if (pfString != null) {
            if (pfString.equals("AStarPathFinding")) {
                pf = new AStarPathFinding();
            }
            if (pfString.equals("BFSPathFinding")) {
                pf = new BFSPathFinding();
            }
            if (pfString.equals("FloodFillPathFinding")) {
                pf = new FloodFillPathFinding();
            }
            if (pfString.equals("GreedyPathFinding")) {
                pf = new GreedyPathFinding();
            }
        }
        switch (e.getName()) {
            case "Attack":
                return new Attack(gs.getUnit(Long.parseLong(e.getAttributeValue("unitID"))),
                    gs.getUnit(Long.parseLong(e.getAttributeValue("target"))), pf);
            case "Build":
                return new Build(gs.getUnit(Long.parseLong(e.getAttributeValue("unitID"))),
                    ...
                    Integer.parseInt(e.getAttributeValue("y")), pf);
            case "Harvest":
                return new Harvest(gs.getUnit(Long.parseLong(e.getAttributeValue("unitID"))),
                    ...
                    gs.getUnit(Long.parseLong(e.getAttributeValue("base"))), pf);
            case "Idle":
