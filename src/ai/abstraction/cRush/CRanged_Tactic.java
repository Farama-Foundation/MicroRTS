...
public class CRanged_Tactic extends AbstractAction {
    ...
    public CRanged_Tactic(Unit u, Unit a_target, Unit h, Unit eb, PathFinding a_pf,
        UnitTypeTable ut, Player pl) {
        super(u);
        target = a_target;
        pf = a_pf;
        home = h;
    ...
    public UnitAction execute(GameState gs, ResourceUsage ru) {
        ...
        for (Unit u2 : gameUnites) {
            ...
            if (u2 != null && u2.getPlayer() != p.getID() && (u2.getType() == rangedType
                || u2.getType() == heavyType || u2.getType() == lightType)) {
                enemyAttackUnits++;
            }
        ...
        if (unit.getType() == workerType) {
            ...
            if (d <= unit.getAttackRange()) {
                return new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, target.getX(),
                    target.getY());
            } else if (timeToAttack) {
                move = pf.findPathToPositionInRange(unit,
                    target.getX() + target.getY() * gs.getPhysicalGameState().getWidth(),
                    unit.getAttackRange(), gs, ru);
            } else if (ally != null) {
                ...
                if (d > ad) {
                    ...
                    move = pf.findPathToPositionInRange(unit,
                        ally.getX() + ally.getY() * gs.getPhysicalGameState().getWidth(),
                        unit.getAttackRange(), gs, ru);
                }
                if (move == null) {
                    move = pf.findPathToPositionInRange(unit,
                        (ally.getX() - 1) + (ally.getY()) * gs.getPhysicalGameState().getWidth(),
                        unit.getAttackRange() + 1, gs, ru);
                }
                if (move == null) {
                    move = pf.findPathToPositionInRange(unit,
                        target.getX() + target.getY() * gs.getPhysicalGameState().getWidth(),
        ...
        if (d <= unit.getAttackRange()) {
            ...
            if (timeToAttack && (target.getType() == baseType)) {
                ...
                move = pf.findPathToPositionInRange(unit,
                    enemyBase.getX() + enemyBase.getY() * gs.getPhysicalGameState().getWidth(),
                    unit.getAttackRange(), gs, ru);
            }
            if (move != null && gs.isUnitActionAllowed(unit, move)) {
                return move;
            ...
            //Attack behavior
            if (d <= (unit.getAttackRange()) - 1 && rd > 2 && unit.getMoveTime() < target
                .getMoveTime()) {
                UnitAction move = pf.findPathToPositionInRange(unit,
                    home.getX() + home.getY() * gs.getPhysicalGameState().getWidth(),
            ...
            if ((distanceWithoutUnits(ally.getX(), (ally.getY() + 1), enemyBase.getX(),
                enemyBase.getY()) > distance(ally, enemyBase))) {
                while (!positionFound) {
                    if ((atDown != null && unit != atDown) && (atDownRight != null
                        && unit != atDownRight) && (atRight != null && unit != atRight)) {
                        ally = atRight;
                    } else {
                        positionFound = true;
                    }
    ...
    public UnitAction squareMove(GameState gs, ResourceUsage ru, Unit targetUnit) {
        ...
        UnitAction moveToUp = pf.findPath(unit,
            (ally.getX()) + (ally.getY() - 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
        UnitAction moveToUpLeft = pf.findPath(unit,
            (ally.getX() - 1) + (ally.getY() - 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
        UnitAction moveToLeft = pf.findPath(unit,
            (ally.getX() - 1) + (ally.getY()) * gs.getPhysicalGameState().getWidth(), gs, ru);
        UnitAction moveToDown = pf.findPath(unit,
            (ally.getX()) + (ally.getY() + 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
        UnitAction moveToDownRight = pf.findPath(unit,
            (ally.getX() + 1) + (ally.getY() + 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
        UnitAction moveToRight = pf.findPath(unit,
            (ally.getX() + 1) + (ally.getY()) * gs.getPhysicalGameState().getWidth(), gs, ru);

        if (distanceWithoutUnits(ally.getX(), (ally.getY() + 1), enemyBase.getX(), enemyBase.getY())
            > distance(ally, enemyBase)) {
            UnitAction move = null;
    ...
    public Unit farthestRangedAlly(Unit start, List<Unit> unites, GameState gs) {
        ...
        for (Unit u2 : unites) {
            if (u2.getType() == rangedType && u2.getPlayer() == p.getID() && home != null) {
