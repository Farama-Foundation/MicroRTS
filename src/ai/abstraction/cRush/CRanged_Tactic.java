...
public class CRanged_Tactic extends AbstractAction {
  ...
  public CRanged_Tactic(Unit u, Unit a_target, Unit h, Unit eb, PathFinding a_pf, UnitTypeTable ut,
      Player pl) {
    super(u);
    target = a_target;
    pf = a_pf;
  ...
  public void toxml(XMLWriter w) {
    w.tagWithAttributes("Attack",
        "unitID=\"" + getUnit().getID() + "\" target=\"" + target.getID() + "\" pathfinding=\"" + pf
  ...
  public UnitAction squareMove(GameState gs, ResourceUsage ru, Unit targetUnit) {
    ...
    UnitAction moveToUp = pf
        .findPath(unit, (ally.getX()) + (ally.getY() - 1) * gs.getPhysicalGameState().getWidth(),
            gs, ru);
    UnitAction moveToUpLeft = pf.findPath(unit,
        (ally.getX() - 1) + (ally.getY() - 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
    UnitAction moveToLeft = pf
        .findPath(unit, (ally.getX() - 1) + (ally.getY()) * gs.getPhysicalGameState().getWidth(),
            gs, ru);
    UnitAction moveToDown = pf
        .findPath(unit, (ally.getX()) + (ally.getY() + 1) * gs.getPhysicalGameState().getWidth(),
            gs, ru);
    UnitAction moveToDownRight = pf.findPath(unit,
        (ally.getX() + 1) + (ally.getY() + 1) * gs.getPhysicalGameState().getWidth(), gs, ru);
    UnitAction moveToRight = pf
        .findPath(unit, (ally.getX() + 1) + (ally.getY()) * gs.getPhysicalGameState().getWidth(),
            gs, ru);
