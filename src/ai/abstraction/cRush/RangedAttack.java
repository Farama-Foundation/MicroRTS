...
public class RangedAttack extends AbstractAction {
    ...
    public void toxml(XMLWriter w) {
        w.tagWithAttributes("RangedAttack",
            "unitID=\"" + getUnit().getID() + "\" target=\"" + target.getID() + "\" pathfinding=\""
                + pf.getClass().getSimpleName() + "\" racks=\"" + racks.getID() + "\"");
        w.tag("/RangedAttack");
    }
    ...
    public UnitAction execute(GameState gs, ResourceUsage ru) {
        ...
        if (d <= (getUnit().getAttackRange()) - 1 && rd > 2 && getUnit().getMoveTime() < target
            .getMoveTime()) {
            UnitAction move = pf.findPathToPositionInRange(getUnit(),
                racks.getX() + racks.getY() * gs.getPhysicalGameState().getWidth(),
