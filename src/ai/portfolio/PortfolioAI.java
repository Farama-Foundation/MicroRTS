...
public class PortfolioAI extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public PortfolioAI(UnitTypeTable utt) {
        this(new AI[]{new WorkerRush(utt), new LightRush(utt), new RangedRush(utt),
                new RandomBiasedAI()}, new boolean[]{true, true, true, false}, 100, -1, 100,
            new SimpleSqrtEvaluationFunction3());
    }

    public PortfolioAI(AI[] s, boolean[] d, int time, int max_playouts, int la,
        ...
        do {
            ...
            for (int i = 0; i < n && !timeout; i++) {
                for (int j = 0; j < n && !timeout; j++) {
                    if (counts[i][j] == 0 || !deterministic[i] || !deterministic[j]) {
                        anyChange = true;
                        ...
                        scores[i][j] += evaluation
                            .evaluate(playerForThisComputation, 1 - playerForThisComputation, gs2);
                        counts[i][j]++;
                        nplayouts++;
                    }
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        if (DEBUG >= 1) {
            System.out
                .println("PortfolioAI: selected " + bestMax + "  with score: " + bestMaxScore);
        }
    ...
    @Override
    public AI clone() {
        return new PortfolioAI(strategies, deterministic, TIME_BUDGET, ITERATIONS_BUDGET, LOOKAHEAD,
            evaluation);
    }
