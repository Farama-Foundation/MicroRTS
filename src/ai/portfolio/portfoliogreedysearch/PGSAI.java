 ...
 * Here: - getSeedPlayer does not make sense in general, since each unit type might have a different
 * set of scripts, so it's ignored - the portfolios might be very large, since we have to include
 * scripts for training, building, harvesting, etc. - new units might be created, so a script is
 * selected as the "default" for those new units before hand
 */
public class PGSAI extends AIWithComputationBudget {
    ...
    public PGSAI(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e,
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        ...
        if (DEBUG >= 1) {
            System.out.println(
                "PGSAI " + player + "(MAX_TIME = " + TIME_BUDGET + ", I: " + I + ", R: " + R + ")");
        }
    ...
    public void improve(int player, UnitScript[] scriptsToImprove, List<Unit> units,
        UnitScript[] otherScripts, List<Unit> otherUnits, GameState gs) throws Exception {
        for (int i = 0; i < I; i++) {
            if (DEBUG >= 1) {
                System.out.println("Improve player " + player + "(" + i + "/" + I + ")");
            }
            for (int u = 0; u < scriptsToImprove.length; u++) {
                ...
                for (UnitScript us : candidates) {
                    ...
                    if (s != null) {
                        ...
                        double e = playout(player, scriptsToImprove, units, otherScripts,
                            otherUnits, gs);
                        if (DEBUG >= 2) {
                            System.out.println(
                                "  " + unit + " -> " + s.getClass().toString() + " -> " + e);
                        }
    ...
    public double playout(int player, UnitScript[] scripts1, List<Unit> units1,
