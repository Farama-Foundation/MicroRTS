...
public class Clause {
        ...
        switch (type) {
            ...
            case CLAUSE_AND: {
                ...
                while (true) {
                    if (DEBUG >= 1) {
                        System.out.println(
                            "Clause.firstMatch(AND): current = " + matches_current + " (previous: "
                                + matches_previous + ")");
                    }
                    ...
                    if (matches_previous < matches_current) {
                        ...
                        if (matches_l != null) {
                            matches_resolved[matches_current] = c.resolve(matches_l, gs);
                        }
                        if (DEBUG >= 1) {
                            System.out.println("Clause.firstMatch(AND): resolved clause = "
                                + matches_resolved[matches_current]);
                        }
                    ...
                    if (l2 == null) {
                        ...
                        // undo bindings:
                        while (matches_l.size() > matches_trail[matches_current]) {
                            matches_l.remove(matches_l.size() - 1);
                        }
                    } else {
                        // success:
    ...
    public List<Binding> nextMatch(GameState gs) throws Exception {
        ...
        if (matches_left == null) {
            return firstMatch(gs);
        }
