...
public class MethodDecomposition {
    ...
    */ int executionState = 0;
    ...
    public void printDetailed(int tabs) {
        ...
        switch (type) {
            case METHOD_CONDITION:
                System.out.println(
                    this.hashCode() + " - " + executionState + " - (:condition " + clause + ")");
                break;
            case METHOD_NON_BRANCHING_CONDITION:
                System.out.println(
                    this.hashCode() + " - " + executionState + " - (:!condition " + clause + ")");
            ...
            case METHOD_OPERATOR:
                if (updatedTerm != null) {
                    System.out.println(
                        this.hashCode() + " - " + executionState + " - (:operator " + updatedTerm
                            + ")");
                } else {
                    System.out.println(
                        this.hashCode() + " - " + executionState + " - (:operator " + term + ")");
                }
                break;
            case METHOD_METHOD:
                if (method != null) {
                    System.out.println(
                        this.hashCode() + " - " + executionState + " - (:method " + method.head
                            + ")");
                    for (int j = 0; j < tabs; j++) {
                        System.out.print("  ");
                    }
                    ...
                    System.out.println(
                        this.hashCode() + " - " + executionState + " - (:method " + term + ")");
                }
                break;
            case METHOD_SEQUENCE: {
    ...
    public MethodDecomposition clone() {
        ...
        c.type = type;
        if (clause != null) {
            c.clause = clause.clone();
        }
        if (term != null) {
            c.term = term.clone();
        }
        if (updatedTerm != null) {
            c.updatedTerm = updatedTerm.clone();
        }
    ...
    public MethodDecomposition cloneTrackingDescendants(MethodDecomposition[] descendantsToTrack,
        ...
        if (subelements != null) {
            c.subelements = new MethodDecomposition[subelements.length];
            for (int i = 0; i < subelements.length; i++) {
                c.subelements[i] = subelements[i]
                    .cloneTrackingDescendants(descendantsToTrack, newDescendants);
            }
        }
        if (method != null) {
    ...
    public void renameVariables(int renamingIndex) {
        ...
        if (term != null) {
            term.renameVariables(renamingIndex);
        }
        if (updatedTerm != null) {
            updatedTerm.renameVariables(renamingIndex);
        }
        if (subelements != null) {
            for (int i = 0; i < subelements.length; i++) {
    ...
    public void applyBindings(List<Binding> l) throws Exception {
        ...
        if (term != null) {
            term.applyBindings(l);
        }
        if (updatedTerm != null) {
            updatedTerm.applyBindings(l);
        }
        if (subelements != null) {
            for (int i = 0; i < subelements.length; i++) {
    ...
    public int executionCycle(GameState gs, List<MethodDecomposition> actions,
        List<MethodDecomposition> choicePoints) {
        switch (type) {
            case METHOD_CONDITION:
            case METHOD_NON_BRANCHING_CONDITION:
                if (executionState == 0) {
            ...
            case METHOD_PARALLEL: {
                ...
                for (int i = 0; i < subelements.length; i++) {
                    ...
                    if (tmp == EXECUTION_ACTION_ISSUE) {
                        anyActionIssue = true;
                    }
                    if (tmp == EXECUTION_CHOICE_POINT || tmp == EXECUTION_FAILURE) {
                        return tmp;
                    }
                    if (tmp != EXECUTION_SUCCESS) {
                        allSuccess = false;
    ...
    */
    public int executionCycle(GameState gs, List<MethodDecomposition> actions,
        List<MethodDecomposition> choicePoints, AdversarialChoicePoint previous_cp) {
        switch (type) {
            case METHOD_CONDITION:
            case METHOD_NON_BRANCHING_CONDITION:
                if (executionState == 0) {
            ...
            case METHOD_SEQUENCE:
                do {
                    ...
                    int tmp = subelements[executionState]
                        .executionCycle(gs, actions, choicePoints, previous_cp);
                    if (tmp == EXECUTION_SUCCESS) {
                        previous_cp.captureExecutionStateNonRecursive(this);
                        executionState++;
                    } else {
    ...
    public void convertToOperatorList(List<Pair<Integer, List<Term>>> l) throws Exception {
        switch (type) {
            ...
            case METHOD_OPERATOR:
                if (updatedTerm != null) {
                    if (l.isEmpty()) {
                        Pair<Integer, List<Term>> tmp = new Pair<>(updatedTermCycle,
                            new ArrayList<>());
            ...
            case METHOD_METHOD:
                if (method != null) {
                    method.getDecomposition().convertToOperatorList(l);
                }
                break;
            case METHOD_SEQUENCE:
            case METHOD_PARALLEL:
    ...
    public void countVariableAppearances(HashMap<Symbol, Integer> appearances) throws Exception {
        if (clause != null) {
            clause.countVariableAppearances(appearances);
        }
        if (term != null) {
            term.countVariableAppearances(appearances);
        }
        if (subelements != null) {
            for (MethodDecomposition md : subelements) {
    ...
    public void replaceSingletonsByWildcards(List<Symbol> singletons) throws Exception {
        if (clause != null) {
            clause.replaceSingletonsByWildcards(singletons);
        }
        if (term != null) {
            term.replaceSingletonsByWildcards(singletons);
        }
        if (subelements != null) {
            for (MethodDecomposition md : subelements) {
