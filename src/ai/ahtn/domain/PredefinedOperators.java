...
public class PredefinedOperators {
    ...
    static {
        try {
            operators.put(new Symbol("!wait"), new OperatorExecutor() {
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    PlayerAction pa) throws Exception {
            ...
            operators.put(new Symbol("!fill-with-idles"), new OperatorExecutor() {
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (pa == null) {
                        ...
                        for (Unit u : gs.getUnits()) {
                            if (u.getPlayer() == player) {
                                if (pa.getAction(u) == null && gs.getUnitAction(u) == null) {
                                    pa.addUnitAction(u, new UnitAction(UnitAction.TYPE_NONE, 10));
                                }
                            }
                        }
                ...
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (state.getOperatorExecutingState() == 1) {
                        ...
                        if (pa == null) {
                            pa = new PlayerAction();
                            pa.addUnitAction(u1,
                                new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, u2.getX(),
                                    u2.getY()));
                            gs.issue(pa);
                        } else {
                            pa.addUnitAction(u1,
                    ...
                    if (state.getOperatorExecutingState() == 1) {
                        ...
                        if (pa == null) {
                            ...
                            if (u1.getX() == u2.getX() - 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST,
                                    UnitAction.DIRECTION_RIGHT));
                            }
                            if (u1.getX() == u2.getX() + 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST,
                                    UnitAction.DIRECTION_LEFT));
                            }
                            if (u1.getY() == u2.getY() - 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST,
                                    UnitAction.DIRECTION_DOWN));
                            }
                            if (u1.getY() == u2.getY() + 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_HARVEST,
                                    UnitAction.DIRECTION_UP));
                            }
                            gs.issue(pa);
                        } else {
                            if (u1.getX() == u2.getX() - 1) {
                ...
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (state.getOperatorExecutingState() == 1) {
                        ...
                        if (pa == null) {
                            pa = new PlayerAction();
                            if (u1.getX() == u2.getX() - 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN,
                                    UnitAction.DIRECTION_RIGHT));
                            }
                            if (u1.getX() == u2.getX() + 1) {
                                pa.addUnitAction(u1, new UnitAction(UnitAction.TYPE_RETURN,
            ...
            operators.put(new Symbol("!produce"), new OperatorExecutor() {
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (state.getOperatorExecutingState() == 1) {
                        ...
                        if (posx >= 0 && posx < gs.getPhysicalGameState().getWidth() && posy >= 0
                            && posy < gs.getPhysicalGameState().getHeight() && gs.free(posx, posy)
                            && gs.getPlayer(u1.getPlayer()).getResources() - ru
                            .getResourcesUsed(u1.getPlayer()) >= ut.cost) {
                            if (pa == null) {
                                pa = new PlayerAction();
                                pa.addUnitAction(u1,
                                    new UnitAction(UnitAction.TYPE_PRODUCE, direction, ut));
                                gs.issue(pa);
                            } else {
                                UnitAction ua = new UnitAction(UnitAction.TYPE_PRODUCE, direction,
                                    ut);
            ...
            operators.put(new Symbol("!move-into-attack-range"), new OperatorExecutor() {
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (gs.getUnitAction(u1) == null) {
                        ...
                        UnitAction ua = pf.findPathToPositionInRange(u1,
                            u2.getPosition(gs.getPhysicalGameState()), u1.getType().attackRange, gs,
                            ru);
                        if (ua != null) {
                            if (pa == null) {
                                pa = new PlayerAction();
            ...
            operators.put(new Symbol("!move-into-harvest-range"), new OperatorExecutor() {
                public boolean execute(Term t, MethodDecomposition state, GameState gs,
                    ...
                    if (gs.getUnitAction(u1) == null) {
                        ...
                        UnitAction ua = pf.findPathToPositionInRange(u1,
                            u2.getPosition(gs.getPhysicalGameState()), 1, gs, ru);
                        if (ua != null) {
                            if (pa == null) {
                                pa = new PlayerAction();
                                pa.addUnitAction(u1, ua);
    ...
    public static boolean execute(MethodDecomposition state, GameState gs, PlayerAction pa)
        throws Exception {
        Term t = state.updatedTerm;
        if (t == null) {
            t = state.term;
