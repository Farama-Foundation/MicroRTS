...
public class Term {
    ...
    public List<Binding> simpleUnificationDestructiveNoSharedVariables(Term t, GameState gs)
        throws Exception {
        if (DEBUG >= 1) {
            System.out.println("simpleUnificationDestructiveNoSharedVariables: start");
        }
        if (!functor.equals(t.functor)) {
            return null;
        }
        if (parameters.length != t.parameters.length) {
            return null;
        }
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (DEBUG >= 1) {
                System.out.println(
                    "simpleUnificationDestructiveNoSharedVariables: " + i + " -> " + p1 + " U "
                        + p2);
            }
    ...
    public boolean equals(Object o) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            if (!parameters[i].equals(t.parameters[i])) {
                return false;
            }
        }
