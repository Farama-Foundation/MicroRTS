        ...
        try {
            predicates.put(new Symbol("="), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    if (p1 instanceof Variable) {
                        if (!((p2 instanceof Variable) && p2.equals(p1))) {
            ...
            predicates.put(new Symbol("closest-unit-to"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    for (Unit u : gs.getUnits()) {
                        ...
                        int d = Math.abs(u.getX() - referenceUnit.getX()) + Math
                            .abs(u.getY() - referenceUnit.getY());
            ...
            predicates.put(new Symbol("can-produce"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    if (DEBUG >= 1) {
                        System.out
                            .println("can-produce.firstMatch: " + Arrays.toString(term.parameters));
                    }
                    ...
                    if ((p1 instanceof SymbolConstant)) {
                        ...
                        if (ut1 != null) {
                            if ((p2 instanceof SymbolConstant)) {
                                ...
                                if (ut2 != null && ut1.produces.contains(ut2)) {
                                    return new LinkedList<>();
                                }
                                ...
                                for (UnitType t : ut1.produces) {
                                    ...
                                    if (!((Variable) p2).ignore()) {
                                        l.add(
                                            new Binding((Variable) p2, new SymbolConstant(t.name)));
                                    }
                                    return l;
                                }
                            }
                        ...
                        if ((p2 instanceof SymbolConstant)) {
                            ...
                            for (UnitType t : gs.getUnitTypeTable().getUnitTypes()) {
                                if (t.produces.contains(ut2)) {
                                    ...
                                    if (!((Variable) p1).ignore()) {
                                        l.add(
                                            new Binding((Variable) p1, new SymbolConstant(t.name)));
                                    }
                                    return l;
                                }
                            }
                            ...
                            for (UnitType t : gs.getUnitTypeTable().getUnitTypes()) {
                                for (UnitType t2 : t.produces) {
                                    ...
                                    if (!((Variable) p2).ignore()) {
                                        l.add(new Binding((Variable) p2,
                                            new SymbolConstant(t2.name)));
                                    }
                                    return l;
                                }
                            }
                ...
                public List<List<Binding>> allMatches(Term term, GameState gs) throws Exception {
                    if (DEBUG >= 1) {
                        System.out
                            .println("can-produce.allMatches: " + Arrays.toString(term.parameters));
                    }
                    List<List<Binding>> ll = new LinkedList<>();
                    Parameter p1 = term.parameters[0];
                    Parameter p2 = term.parameters[1];
            ...
            predicates.put(new Symbol("has-resources-to-produce"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    if ((p1 instanceof IntegerConstant) && (p2 instanceof SymbolConstant)) {
                        ...
                        UnitType ut = gs.getUnitTypeTable().getUnitType(p2.toString());
                        ResourceUsage ru = gs.getResourceUsage();
                        if (player1.getResources() - ru.getResourcesUsed(player1.getID())
                            >= ut.cost) {
                            return new LinkedList<>();
                        }
                    }
            ...
            predicates.put(new Symbol("direction"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    if (p instanceof IntegerConstant) {
                        ...
                        if (d == UnitAction.DIRECTION_UP || d == UnitAction.DIRECTION_RIGHT
                            || d == UnitAction.DIRECTION_DOWN || d == UnitAction.DIRECTION_LEFT) {
                            return new LinkedList<>();
                        }
                    } else {
                        ...
                        if (!((Variable) p).ignore()) {
                            l.add(new Binding((Variable) p,
                                new IntegerConstant(UnitAction.DIRECTION_UP)));
                        }
                    }
                    return null;
                }
                ...
                public List<List<Binding>> allMatches(Term term, GameState gs) throws Exception {
                    ...
                    if (p instanceof IntegerConstant) {
                        ...
                        if (!((Variable) p).ignore()) {
                            l.add(new Binding((Variable) p,
                                new IntegerConstant(UnitAction.DIRECTION_RIGHT)));
                        }
                        ll.add(l);
                        l = new LinkedList<>();
                        if (!((Variable) p).ignore()) {
                            l.add(new Binding((Variable) p,
                                new IntegerConstant(UnitAction.DIRECTION_DOWN)));
                        }
                        ll.add(l);
                        l = new LinkedList<>();
                        if (!((Variable) p).ignore()) {
                            l.add(new Binding((Variable) p,
                                new IntegerConstant(UnitAction.DIRECTION_LEFT)));
                        }
                        ll.add(l);
                    }
                    return ll;
                    ...
                    if (p1 instanceof IntegerConstant) {
                        ...
                        if (p2 instanceof IntegerConstant) {
                            ...
                            if (posx >= 0 && posx < gs.getPhysicalGameState().getWidth()
                                && posy >= 0 && posy < gs.getPhysicalGameState().getHeight() && gs
                                .free(posx, posy)) {
                                return new LinkedList<>();
                            }
                        } else if (p2 instanceof Variable) {
            ...
            predicates.put(new Symbol("next-available-unit"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    for (Unit u : gs.getUnits()) {
                        if (u.getPlayer() == player && u.getID() > lastunit
                            && gs.getUnitAction(u) == null) {
                            if (found == null) {
                                found = u;
                            } else {
                                if (u.getID() < found.getID()) {
                                    found = u;
                                }
                            }
                        }
            ...
            predicates.put(new Symbol("path"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    if ((p1 instanceof IntegerConstant) && (p2 instanceof IntegerConstant)) {
                        ...
                        if (pf.pathToPositionInRangeExists(u1,
                            u2.getPosition(gs.getPhysicalGameState()), 1, gs, null)) {
                            return new LinkedList<>();
                        } else {
                            return null;
                        }
            ...
            predicates.put(new Symbol("path-to-attack"), new PredicateTester() {
                public List<Binding> firstMatch(Term term, GameState gs) throws Exception {
                    ...
                    if ((p1 instanceof IntegerConstant) && (p2 instanceof IntegerConstant)) {
                        ...
                        if (pf.pathToPositionInRangeExists(u1,
                            u2.getPosition(gs.getPhysicalGameState()), u1.getAttackRange(), gs,
                            null)) {
