...
public class AdversarialChoicePoint {
    ...
    public AdversarialChoicePoint(MethodDecomposition cpMax, MethodDecomposition cpMin,
        MethodDecomposition rootMax, MethodDecomposition rootMin, GameState a_gs, int nops,
        int ltoi) {
        maxPlanRoot = rootMax;
        minPlanRoot = rootMin;
    ...
    public AdversarialChoicePoint(MethodDecomposition cpMax, MethodDecomposition cpMin,
        MethodDecomposition rootMax, MethodDecomposition rootMin, GameState a_gs, int nops,
        int ltoi, float a, float b, boolean saveExecutionState) {
        maxPlanRoot = rootMax;
        minPlanRoot = rootMin;
        choicePointPlayerMax = cpMax;
        choicePointPlayerMin = cpMin;
    ...
    public void restoreAfterPop() {
        if (choicePointPlayerMax != null
            && choicePointPlayerMax.getType() == MethodDecomposition.METHOD_METHOD) {
            choicePointPlayerMax.setMethod(null);
        }
        if (choicePointPlayerMin != null
            && choicePointPlayerMin.getType() == MethodDecomposition.METHOD_METHOD) {
    ...
    public boolean nextExpansion(DomainDefinition dd, List<Binding> bindings, int renamingIndex,
        ...
        if (choicePointPlayerMax != null) {
            ...
            if (choicePointPlayerMax.getType() == MethodDecomposition.METHOD_METHOD) {
                ...
                || choicePointPlayerMax.getType()
                == MethodDecomposition.METHOD_NON_BRANCHING_CONDITION) {
    ...
    private boolean nextConditionExpansion(MethodDecomposition choicePoint, List<Binding> bindings)
        ...
        if (DEBUG >= 1) {
            System.out.println(
                "AdversarialChoicePoint.nextExpansionCondition: testing " + choicePoint
                    .getClause());
        }
    ...
    public void setMethod(MethodDecomposition choicePoint, HTNMethod original, GameState gs,
        List<Binding> bindings, int renamingIndex) throws Exception {
        // 1. Clone method:
        HTNMethod m = original.clone();
        choicePoint.setMethod(m);

        // 2. Find variable bindings:
        List<Binding> bindings2 = m.getHead()
            .simpleUnificationDestructiveNoSharedVariables(choicePoint.getTerm(), gs);
        if (DEBUG >= 1) {
    ...
    public boolean processEvaluation(float f, MethodDecomposition maxPlan,
        MethodDecomposition minPlan, boolean clone) {
        switch (minimaxType) {
            case -1:
                if (choicePointPlayerMax != null) {
