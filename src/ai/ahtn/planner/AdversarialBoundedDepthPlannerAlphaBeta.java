...
public class AdversarialBoundedDepthPlannerAlphaBeta {
    ...
    */
    ...
    public Pair<MethodDecomposition, MethodDecomposition> getBestPlan(long timeLimit,
        int maxPlayouts, boolean forceAnswer) throws Exception {
        if (DEBUG >= 1) {
        ...
        if (stack == null) {
            if (DEBUG >= 1) {
                System.out.println(
                    "AdversarialBoundedDepthPlanner.getBestPlan: first time, initializing stack");
            }
            stack = new ArrayList<>();
            stack.add(0,
                new AdversarialChoicePoint(maxPlanRoot, minPlanRoot, maxPlanRoot, minPlanRoot, gs,
                    0, -1, -EvaluationFunction.VICTORY, EvaluationFunction.VICTORY, false));
            trail = new ArrayList<>();
            trail.add(0, 0);
        ...
        do {
            if ((timeLimit > 0 && System.currentTimeMillis() >= timeLimit) || (maxPlayouts > 0
                && nPlayouts >= maxPlayouts)) {
            ...
            if (DEBUG >= 2) {
                System.out.println(
                    "\nAdversarialBoundedDepthPlanner.getBestPlan: stack size: " + stack.size()
                        + ", bindings: " + bindings.size() + ", gs.time: " + choicePoint.gs
                        .getTime() + ", operators: " + root.choicePointPlayerMin
                        .getOperatorsBeingExecuted() + ", " + root.choicePointPlayerMax
                        .getOperatorsBeingExecuted());
/*
                if (stack.size()==23) {
    ...
    */
            ...
            if (DEBUG >= 3) {
                System.out
                    .println("AdversarialBoundedDepthPlanner.getBestPlan: bindings: " + bindings);
            ...
            boolean pop = true;
            while (!timeout && choicePoint
                .nextExpansion(dd, bindings, renamingIndex, choicePoint)) {
                renamingIndex++;
                ...
                if (tmp == null) {
                    ...
                    if (DEBUG >= 2) {
                        System.out.println(
                            "AdversarialBoundedDepthPlanner.getBestPlan: plan execution failed");
                    }
                    choicePoint.restoreExecutionState();
                    ...
                    if (DEBUG >= 2) {
                        System.out.println(
                            "AdversarialBoundedDepthPlanner.getBestPlan: plan execution success or depth limit reached");
                    }
                    if (DEBUG >= 3) {
                    ...
                    last_tree_leaves++;
                    boolean alphaBetaTest = choicePoint
                        .processEvaluation(eval, choicePoint.maxPlanRoot, choicePoint.minPlanRoot,
                            true);
                    if (!ALPHA_BETA_CUT) {
                        alphaBetaTest = false;
                    }
                    ...
                    if (DEBUG >= 2) {
                        System.out
                            .println("AdversarialBoundedDepthPlanner.getBestPlan: stack push");
                    }
                    stack.add(0, tmp);
            ...
            if (pop) {
                if (!timeout && nPlayouts == 0) {
                    ...
                    if ((cp.choicePointPlayerMax != null
                        ...
                        || (cp.choicePointPlayerMax == null && cp.choicePointPlayerMin != null
                        && cp.choicePointPlayerMin.getType()
                        == MethodDecomposition.METHOD_METHOD)) {
    ...
    //                        stack.get(0).processEvaluaiton(eval, maxPlanRoot, minPlanRoot);
                            ...
                            */
                            ...
                            pop = stack.get(0).processEvaluation(choicePoint.bestEvaluation,
                                choicePoint.bestMaxPlan, choicePoint.bestMinPlan, false);
                            if (!ALPHA_BETA_CUT) {
                                pop = false;
                            }
                        }
        ...
        */
        ...
        if (DEBUG >= 1) {
            System.out
                .println("AdversarialBoundedDepthPlanner.nextPlan: options exhausted for rootPlan");
        }
        if (DEBUG >= 1) {
    ...
    */
    public AdversarialChoicePoint simulateUntilNextChoicePoint(List<Binding> bindings,
        AdversarialChoicePoint previous_cp) throws Exception {
        ...
        while (true) {
            ...
            if (!SIMULATE_UNTIL_NEXT_CHOICEPOINT) {
                if (operatorDepth >= maxDepth && gs2.getTime() > lastTimeOperatorsIssued) {
                    // max depth reached:
                    return new AdversarialChoicePoint(null, null, previous_cp.maxPlanRoot,
                        previous_cp.minPlanRoot, gs2, operatorDepth, lastTimeOperatorsIssued,
            ...
            int er1 = previous_cp.maxPlanRoot
                .executionCycle(gs2, actions1, choicePoints1, previous_cp);
            int er2 = previous_cp.minPlanRoot
                .executionCycle(gs2, actions2, choicePoints2, previous_cp);
            if (SIMULATE_UNTIL_NEXT_CHOICEPOINT) {
                if (operatorDepth >= maxDepth && gs2.getTime() > lastTimeOperatorsIssued && (
            ...
            if (er1 == MethodDecomposition.EXECUTION_SUCCESS
                ...
                if (DEBUG >= 2) {
                    System.out.println(
                        "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: execution failure "
                            + er1 + ", " + er2);
                }
                return null;
                ...
                if (cp_md == null) {
                    cp_md = (choicePoints2.isEmpty() ? null : choicePoints2.get(0));
                }
                if (cp_md.getType() == MethodDecomposition.METHOD_NON_BRANCHING_CONDITION) {
                    AdversarialChoicePoint acp = new AdversarialChoicePoint(
                        (choicePoints1.isEmpty() ? null : choicePoints1.get(0)),
                        (choicePoints2.isEmpty() ? null : choicePoints2.get(0)),
                        previous_cp.maxPlanRoot, previous_cp.minPlanRoot, gs2, operatorDepth,
                        lastTimeOperatorsIssued, previous_cp.getAlpha(), previous_cp.getBeta(),
                        false);
                    ...
                    return new AdversarialChoicePoint(
                        (choicePoints1.isEmpty() ? null : choicePoints1.get(0)),
                        (choicePoints2.isEmpty() ? null : choicePoints2.get(0)),
                        previous_cp.maxPlanRoot, previous_cp.minPlanRoot, gs2, operatorDepth,
                        lastTimeOperatorsIssued, previous_cp.getAlpha(), previous_cp.getBeta(),
                ...
                if (previous_cp.maxPlanRoot.getOperatorsBeingExecuted() != null) {
                    for (MethodDecomposition md : previous_cp.maxPlanRoot
                        .getOperatorsBeingExecuted()) {
                        previous_cp.captureExecutionStateNonRecursive(md);
                        // issue action:
                        if (DEBUG >= 2) {
                            System.out.println(
                                "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: continuing executing operator "
                                    + md.getUpdatedTerm());
                        }
                        if (PredefinedOperators.execute(md, gs2) || gs2.getTime()
                            > md.getUpdatedTermCycle() + operatorExecutionTimeout) {
                            ...
                            if (DEBUG >= 2) {
                                System.out.println(
                                    "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: operator complete (1).");
                            }
                        } else {
                ...
                if (previous_cp.minPlanRoot.getOperatorsBeingExecuted() != null) {
                    toDelete = null;
                    for (MethodDecomposition md : previous_cp.minPlanRoot
                        .getOperatorsBeingExecuted()) {
                        previous_cp.captureExecutionStateNonRecursive(md);
                        // issue action:
                        if (DEBUG >= 2) {
                        ...
                        if (PredefinedOperators.execute(md, gs2) || gs2.getTime()
                            ...
                            if (DEBUG >= 2) {
                                System.out.println(
                                    "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: operator complete (2).");
                            }
                        } else {
                            md.setExecutionState(1);
                        }
            ...
            if (er1 == MethodDecomposition.EXECUTION_ACTION_ISSUE) {
                for (MethodDecomposition md : actions1) {
                    ...
                    if (DEBUG >= 2) {
                        System.out.println(
                            "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: executing operator "
                                + md.getUpdatedTerm());
                    }
                    ...
                    if (PredefinedOperators.execute(md, gs2)) {
                        ...
                        if (DEBUG >= 2) {
                            System.out.println(
                                "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: operator complete (3).");
                        }
                    } else {
                        md.setExecutionState(1);
                        if (previous_cp.maxPlanRoot.getOperatorsBeingExecuted() == null) {
            ...
            if (er2 == MethodDecomposition.EXECUTION_ACTION_ISSUE) {
                for (MethodDecomposition md : actions2) {
                    ...
                    if (PredefinedOperators.execute(md, gs2)) {
                        ...
                        if (DEBUG >= 2) {
                            System.out.println(
                                "AdversarialBoundedDepthPlanner.simulateUntilNextChoicePoint: operator complete (4).");
                        }
                    } else {
                        md.setExecutionState(1);
                        if (previous_cp.minPlanRoot.getOperatorsBeingExecuted() == null) {
