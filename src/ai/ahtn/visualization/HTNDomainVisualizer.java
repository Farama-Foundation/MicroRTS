...
public class HTNDomainVisualizer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DomainDefinition dd = DomainDefinition
            .fromLispFile("ahtn/microrts-ahtn-definition-portfolio.lisp");
    ...
    public BufferedImage visualizeHTNOperator(Symbol root) throws Exception {
        ...
        BufferedImage img = new BufferedImage(hpadding * 2 + textWidth,
            vpadding * 2 + fm.getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    public BufferedImage visualizeHTNTask(DomainDefinition d, Symbol root,
        List<Symbol> alreadyProcessed) throws Exception {
        if (alreadyProcessed.contains(root)) {
            ...
            BufferedImage img = new BufferedImage(width,
                fm.getHeight() + 2 * vpadding + vMethodPadding + methodHeight,
                BufferedImage.TYPE_INT_ARGB);
            Graphics2D g2d = img.createGraphics();
            g2d.setFont(font);
            if (DEBUG >= 1) {
                System.out.println(
                    "  visualizeHTNTask img: " + img.getWidth() + " x " + img.getHeight()
            ...
            for (BufferedImage img2 : images) {
                ...
                g2d.drawLine(width / 2, fm.getHeight() + vpadding * 2, x + img2.getWidth() / 2,
                    fm.getHeight() + vpadding * 2 + vMethodPadding);
            ...
            g2d.setColor(Color.BLACK);
            g2d.fillRect(width / 2 - (textWidth + 2 * hpadding) / 2, 0, textWidth + 2 * hpadding,
                fm.getHeight() + vpadding * 2);
            g2d.setColor(Color.WHITE);
            g2d.drawString(root.get(), width / 2 - (textWidth + 2 * hpadding) / 2 + hpadding,
                vpadding + fm.getHeight());

            return img;
        }
    }

    public BufferedImage visualizeHTNMethod(DomainDefinition d, HTNMethod m,
        List<Symbol> alreadyProcessed) throws Exception {
        ...
        BufferedImage img = new BufferedImage(width,
            fm.getHeight() + vpadding * 2 + vMethodPadding + childrenHeight,
            BufferedImage.TYPE_INT_ARGB);
        Graphics2D g2d = img.createGraphics();
        g2d.setFont(font);
        if (DEBUG >= 1) {
            System.out
                .println("  visualizeHTNMethod img: " + img.getWidth() + " x " + img.getHeight());
        }
        ...
        g2d.fillRect(width / 2 - (textWidth + 2 * hpadding) / 2, 0, textWidth + 2 * hpadding,
            fm.getHeight() + vpadding * 2);
        g2d.setColor(Color.BLACK);
        g2d.drawString(m.getName(), width / 2 - (textWidth + 2 * hpadding) / 2 + hpadding,
            vpadding + fm.getHeight());
    ...
    */
    public BufferedImage visualizeHTNPlan(HTNMethod m) throws Exception {
        ...
        int width = Math.max(textWidth + 2 * hpadding, childrenWidth);
        BufferedImage img = new BufferedImage(width,
            (fm.getHeight() + vpadding * 2) * 2 + vMethodPadding + childrenHeight,
            BufferedImage.TYPE_INT_ARGB);
        Graphics2D g2d = img.createGraphics();
        g2d.setFont(font);
        if (DEBUG >= 1) {
        ...
        for (BufferedImage img2 : images) {
            g2d.setColor(Color.BLACK);
            g2d.drawLine(width / 2, (fm.getHeight() + vpadding * 2) * 2, x + img2.getWidth() / 2,
                (fm.getHeight() + vpadding * 2) * 2 + vMethodPadding);
        ...
        g2d.fillRect(width / 2 - (textWidth + 2 * hpadding) / 2, 0, textWidth + 2 * hpadding,
            fm.getHeight() + vpadding * 2);
        g2d.setColor(Color.BLACK);
        g2d.drawString(m_head_name, width / 2 - (textWidth + 2 * hpadding) / 2 + hpadding,
            vpadding + fm.getHeight());

        g2d.setColor(Color.BLACK);
        g2d.fillRect(width / 2 - (textWidth + 2 * hpadding) / 2, fm.getHeight() + vpadding * 2,
            textWidth + 2 * hpadding, fm.getHeight() + vpadding * 2);
        g2d.setColor(Color.WHITE);
        g2d.drawString(m_name, width / 2 - (textWidth + 2 * hpadding) / 2 + hpadding,
            fm.getHeight() + vpadding * 2 + vpadding + fm.getHeight());

        return img;
    }

    public BufferedImage visualizeHTNPlan(MethodDecomposition m) throws Exception {
        if (m.getType() == MethodDecomposition.METHOD_METHOD && m.getMethod() != null) {
            return visualizeHTNPlan(m.getMethod());
        }
