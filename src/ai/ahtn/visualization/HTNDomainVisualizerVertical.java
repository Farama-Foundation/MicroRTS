...
public class HTNDomainVisualizerVertical {
    ...
    public static void main(String[] args) throws Exception {
        {
            DomainDefinition dd = DomainDefinition
                .fromLispFile("ahtn/microrts-ahtn-definition-lowest-level.lisp");
        ...
        {
            DomainDefinition dd = DomainDefinition
                .fromLispFile("ahtn/microrts-ahtn-definition-low-level.lisp");
            HTNDomainVisualizerVertical v = new HTNDomainVisualizerVertical();
            BufferedImage img = v.visualizeHTNDomain(dd, new Symbol("destroy-player"));
        ...
        {
            DomainDefinition dd = DomainDefinition
                .fromLispFile("ahtn/microrts-ahtn-definition-flexible-portfolio.lisp");
            HTNDomainVisualizerVertical v = new HTNDomainVisualizerVertical();
            BufferedImage img = v.visualizeHTNDomain(dd, new Symbol("destroy-player"));
            ImageIO.write(img, "png", new File("HTN-domain-flexible-portfolio.png"));
        }
        {
            DomainDefinition dd = DomainDefinition.fromLispFile(
                "ahtn/microrts-ahtn-definition-flexible-single-target-portfolio.lisp");
            HTNDomainVisualizerVertical v = new HTNDomainVisualizerVertical();
            BufferedImage img = v.visualizeHTNDomain(dd, new Symbol("destroy-player"));
    ...
    public BufferedImage visualizeHTNTask(DomainDefinition d, Symbol root,
        ...
        if (alreadyProcessed.contains(root)) {
            ...
            Graphics2D g2d = img.createGraphics();
            g2d.setFont(font);
            System.out.println("  visualizeHTNTask img: " + img.getWidth() + " x " + img.getHeight()
                + " (methodHeight: " + methodHeight + ")");

            int y = 0;
            for (BufferedImage img2 : images) {
                g2d.setColor(Color.BLACK);
                g2d.drawLine(textWidth + 2 * hpadding, height / 2,
                    textWidth + 2 * hpadding + hMethodPadding, y + img2.getHeight() / 2);

                System.out.println(
                    "  drawing image at: " + (textWidth + 2 * hpadding + hMethodPadding) + ", "
                        + y);
            ...
            g2d.setColor(Color.BLACK);
            g2d.fillRect(0, height / 2 - (fm.getHeight() + 2 * vpadding) / 2,
                textWidth + 2 * hpadding, fm.getHeight() + vpadding * 2);
            g2d.setColor(Color.WHITE);
            g2d.drawString(root.get(), hpadding,
                height / 2 - (fm.getHeight() + 2 * vpadding) / 2 + fm.getHeight() + vpadding - 2);
    ...
    public BufferedImage visualizeHTNMethod(DomainDefinition d, HTNMethod m,
        ...
        g2d.fillRect(0, height / 2 - (fm.getHeight() + 2 * vpadding) / 2, textWidth + 2 * hpadding,
            fm.getHeight() + vpadding * 2);
        g2d.setColor(Color.BLACK);
        g2d.drawString(m.getName(), hpadding,
            height / 2 - (fm.getHeight() + 2 * vpadding) / 2 + fm.getHeight() + vpadding - 2);
    ...
    */
    public BufferedImage visualizeHTNPlan(HTNMethod m) throws Exception {
        ...
        g2d.fillRect(0, height / 2 - (fm.getHeight() + 2 * vpadding) / 2, textWidth + 2 * hpadding,
            fm.getHeight() + vpadding * 2);
        g2d.setColor(Color.BLACK);
        g2d.drawString(m_name, hpadding,
            height / 2 - (fm.getHeight() + 2 * vpadding) / 2 + fm.getHeight() + vpadding - 2);
