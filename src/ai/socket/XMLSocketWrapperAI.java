...
public class XMLSocketWrapperAI {
    ...
    public static void runServer(AIWithComputationBudget ai, int socket) throws Exception {
        if (DEBUG >= 1) {
            System.out.println("XMLSocketWrapperAI server is running.");
        }
        int clientNumber = 0;
        ServerSocket listener = new ServerSocket(socket);
        try {
            while (true) {
                new SocketWrapperAI(listener.accept(), clientNumber++,
                    (AIWithComputationBudget) ai.clone()).start();
            }
        } finally {
            listener.close();
        }
    ...
    private static class SocketWrapperAI extends Thread {
        ...
        public SocketWrapperAI(Socket a_socket, int a_clientNumber, AIWithComputationBudget a_ai) {
            ...
            if (DEBUG >= 1) {
                System.out
                    .println("New connection with client# " + a_clientNumber + " at " + a_socket);
            }
        }
            ...
            try {
                ...
                while (true) {
                    ...
                    if (input.startsWith("end")) {
                        ...
                        if (DEBUG >= 1) {
                            System.out.println(
                                "setting the budget to: " + time_budget + ", " + iterations_budget);
                        }
                        ...
                        utt = UnitTypeTable.fromXML(
                            new SAXBuilder().build(new StringReader(input)).getRootElement());
                        ai.reset(utt);
                        out.append("ack\n");
                        out.flush();
                        ...
                        GameState gs = GameState.fromXML(
                            new SAXBuilder().build(new StringReader(input)).getRootElement(), utt);
                        if (DEBUG >= 1) {
                        ...
                        if (tokens.length >= 2) {
                            ...
                            if (readWriteFolder.startsWith("\"")) {
                                readWriteFolder = readWriteFolder
                                    .substring(1, readWriteFolder.length() - 1);
                            }
                        }
                        if (DEBUG >= 1) {
