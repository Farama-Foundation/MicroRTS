 ...
 * - branchingFactorUpperBound: this is very fast, but only provides an uppower bound Basically
 * computing the number of actions each individual unit can execute, and multiplying these numbers.
 * - branchingFactor: this is exact, but very slow. It enumerates each move. -
 * branchingFactorByResourceUsage: same, but returns a vector, with the number of actions that
 * require different nuber of resources. - branchingFactorByResourceUsageFast/ -
 * branchingFactorByResourceUsageSeparatingFast: these are the fastest methods, the second takes
 * into account that there are groups of units for which we can compute the branching factor
 * separatedly.
 * <p>
...
public class BranchingFactorCalculatorLong {
    ...
    public static long branchingFactor(GameState gs, int player) throws Exception {
        long n = 0;
        PlayerActionGenerator pag = new PlayerActionGenerator(gs, player);
        while (pag.getNextAction(-1) != null) {
            n++;
        }
        return n;
    }

    public static long branchingFactorByResourceUsageSeparatingFast(GameState gs, int player)
        throws Exception {
        ...
        for (int i = 0; i < pgs2.getHeight(); i++) {
            for (int j = 0; j < pgs2.getWidth(); j++) {
                if (separation[j][i] != 0 && !areas.contains(separation[j][i])) {
    ...
    public static long[] branchingFactorByResourceUsageFastInternal(GameState gs, int player)
        throws Exception {
        GameState gs2 = gs.clone();
        PhysicalGameState pgs2 = gs2.getPhysicalGameState();
    ...
    public static long branchingFactorByResourceUsageFast(GameState gs, int player)
        throws Exception {
        int playerResources = gs.getPlayer(player).getResources();
