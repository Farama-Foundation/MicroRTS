...
/**
 * An "AIWithComputationBudget" is one that is given a limit in the amount of CPU it can use per
 * game frame. This limit is specified in either: - TIME_BUDGET: number of milliseconds that the
 * call to "getAction" can take, or - ITERATIONS_BUDGET: number of internal iterations the AI can
 * use (e.g., in a Monte Carlo AI, this is the number of playouts, or in a minimax AI, this is the
 * number of leaves it can explore).
 * <p>
...
public abstract class AIWithComputationBudget extends AI {
    ...
    /**
     * Number of milisseconds the function {@link #getAction(int, rts.GameState)} is allowed to use.
     * If set to -1, time is not limited.
     */
    protected int TIME_BUDGET = 100;

    /**
     * Number of internal iterations for this controller (e.g. playouts for Monte Carlo Tree
     * Search). If set to -1, the number of iterations is not limited
     */
    ...
    /**
     * Returns the number of internal iterations this AI is allowed (-1 means no limit of
     * iterations)
     *
     * @return
     */
