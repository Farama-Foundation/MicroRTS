 ...
 * <p>
 * The idea is that of an "anytime" algorithm: compute until requested to return the best solution
 * so far. Computation is prepared by {@link #startNewComputation}, performed with {@link
 * #computeDuringOneGameFrame} (which can be called several times to improve the current solution)
 * and the best solution found is returned with  {@link #getBestActionSoFar()}.
 * <p>
...
public interface InterruptibleAI {
    ...
    /**
     * Starts the calculation of an action to return, receiving the player to act and the game
     * state.
     *
    ...
    /**
     * Resumes the computation of the best action calculated so far. Every time this function is
     * called is an opportunity to improve the action to be taken in the {@link GameState} received
     * in {@link #startNewComputation}
     *
     * @throws Exception
     */
