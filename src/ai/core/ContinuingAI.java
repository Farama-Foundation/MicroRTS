...
public class ContinuingAI extends AI {
    ...
    /**
     * An {@link AI} instance that implements {@link InterruptibleAI}. ContinuingAI uses this AI to
     * effectively calculate the action to return
     */
    protected AI m_AI;
    ...
    /**
     * Instantiates the ContinuingAI with an AI that implements {@link InterruptibleAI}. Throws an
     * exception if the received AI does not implement {@link InterruptibleAI}.
     *
     * @param ai
     * @throws Exception
     */
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        if (gs.canExecuteAnyAction(player)) {
            ...
            if (m_gameStateUsedForComputation != null && !m_gameStateUsedForComputation
                .equals(gs)) {
                if (DEBUG >= 1) {
                    System.out.println(
            ...
            if (!m_isThereAComputationGoingOn) {
                ...
                while (newGameState.winner() == -1 && !newGameState.gameover() && !newGameState
                    .canExecuteAnyAction(0) && !newGameState.canExecuteAnyAction(1)) {
                    newGameState.cycle();
                }

                // if the reached state is not a game over and this player can act,
                // starts a new computation
                if ((newGameState.winner() == -1 && !newGameState.gameover()) && newGameState
                    .canExecuteAnyAction(player)) {
                    if (DEBUG >= 1) {
                        System.out.println(
                            "ContinuingAI: this cycle we do not need an action, but we will be next to move");
                    }
                    m_isThereAComputationGoingOn = true;
                    m_gameStateUsedForComputation = newGameState;
                    ((InterruptibleAI) m_AI)
                        .startNewComputation(player, m_gameStateUsedForComputation);
                    ((InterruptibleAI) m_AI).computeDuringOneGameFrame();
                } else { // game is over or this player cannot move
                    if (DEBUG >= 1) {
                        System.out.println(
                            "ContinuingAI: this cycle we do not need an action, but we will not be next to move, so we can do nothing");
                    }
                }
            } else { // computation was already started, resume to improve solution
                if (DEBUG >= 1) {
                    System.out
                        .println("ContinuingAI: continuing a computation from a previous frame");
                }
                ((InterruptibleAI) m_AI).computeDuringOneGameFrame();
            }
