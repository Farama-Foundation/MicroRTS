 ...
 * @author santi:
 * <p>
 * - This is the ABCD (Alpha-Beta considering durations) algorithm presented by Churchill and Buro
 * at AIIDE 2012 - In particular, this version uses the "alt" tree alteration technique to improve
 * the estimation of the alphabeta values when there are simultaneous moves.
 */
public class ABCD extends AI {
    ...
    public MiniMaxResult ABCD(GameState gs, int maxplayer, int minplayer, float alpha, float beta,
        int depthLeft, int nextPlayerInSimultaneousNode) throws Exception {
        ...
        if (toMove == maxplayer) {
            ...
            do {
                ...
                if (next != null) {
                    ...
                    MiniMaxResult tmp = ABCD(gs2, maxplayer, minplayer, alpha, beta, depthLeft - 1,
            ...
            GameState gs2 = gs.clone();
            while (gs2.winner() == -1 && !gs2.gameover() && !gs2.canExecuteAnyAction(maxplayer)
                && !gs2.canExecuteAnyAction(minplayer)) {
                gs2.cycle();
            }
