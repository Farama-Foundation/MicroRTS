...
public class IDABCD extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public IDABCD(UnitTypeTable utt) {
        this(100, -1, new WorkerRush(utt, new AStarPathFinding()), 100,
            new SimpleSqrtEvaluationFunction3(), true);
    }

    public IDABCD(int tpc, int ppc, AI a_playoutAI, int a_maxPlayoutTime, EvaluationFunction a_ef,
        boolean a_performGreedyActionScan) {
        super(tpc, ppc);
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        do {
            ...
            PlayerAction tmp = searchOutsideStack(gs_to_start_from, maxplayer, minplayer, depth,
                cutOffTime, ITERATIONS_BUDGET, false);
            if (DEBUG >= 1) {
            ...
            if (stack.isEmpty()) {
                ...
                if (nNodes > max_nodes_so_far) {
                    max_nodes_so_far = nNodes;
                }
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        if (last_time_depth > max_time_depth_so_far) {
            max_time_depth_so_far = last_time_depth;
        }

        if (bestMove == null) {
            PlayerActionGenerator pag = new PlayerActionGenerator(gs_to_start_from,
    ...
    public PlayerAction greedyActionScan(GameState gs, int player, long cutOffTime, int maxPlayouts)
        throws Exception {
    ...
    public PlayerAction searchOutsideStack(GameState initial_gs, int maxplayer, int minplayer,
        int depth, long cutOffTime, int maxPlayouts, boolean needAResult) throws Exception {
        ABCDNode head;
        if (stack == null) {
            ...
            head = new ABCDNode(-1, 0, initial_gs, -EvaluationFunction.VICTORY,
                EvaluationFunction.VICTORY, 0);
            stack.add(head);
            treeIsComplete = true;
        } else {
        ...
        while (!stack.isEmpty()) {
            ...
            if (maxPlayouts > 0 && nPlayouts >= maxPlayouts) {
                break;
            }
            ...
            switch (current.type) {
                case -1: // unknown node:
                {
                    ...
                    if (current.depth >= depth || winner != -1 || gameover) {
                        ...
                        lastResult = new Pair<PlayerAction, Float>(null,
                            ef.evaluate(maxplayer, minplayer, gs2));
                ...
                case 0: // max node:
                    ...
                    if (current.actions == null) {
                        ...
                        if (DEBUG >= 2) {
                            ...
                            System.out.println(
                                "PlayerGenerator moves: " + l + "(cutOffTime: " + cutOffTime + ")");
                        }
                        ...
                        if (next != null) {
                            ...
                            stack.add(0, new ABCDNode(-1, current.depth + 1, gs2, current.alpha,
                                current.beta, current.nextPlayerInSimultaneousNode));
                        } else {
                        ...
                        if (DEBUG >= 2) {
                            ...
                            System.out.println(
                                "alpha: " + current.alpha + ", beta: " + current.beta + ", next: "
                                    + next);
                        }
                ...
                case 2: // simulation node:
                    ...
                    while (current.gs.winner() == -1 && !current.gs.gameover() && !current.gs
                        .canExecuteAnyAction(maxplayer) && !current.gs
