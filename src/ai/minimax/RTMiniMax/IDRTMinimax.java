...
public class IDRTMinimax extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        do {
            ...
            PlayerAction tmp = timeBoundedRealTimeMinimaxABOutsideStack(gs_to_start_from, maxplayer,
                minplayer, gs_to_start_from.getTime() + lookAhead, cutOffTime, false);
            if (tmp != null) {
                bestMove = tmp;
            ...
            if (stack.isEmpty()) {
                ...
                int nextLookAhead = Math
                    .max((minCT + 1) - gs_to_start_from.getTime(), lookAhead + 4);
    ...
    public PlayerAction timeBoundedRealTimeMinimaxABOutsideStack(GameState initial_gs,
        ...
        if (stack == null) {
            ...
            head = new RTMiniMaxNode(0, initial_gs, -EvaluationFunctionForwarding.VICTORY,
                EvaluationFunctionForwarding.VICTORY);
            stack.add(head);
        } else {
            if (stack.isEmpty()) {
