...
public class RTMinimax extends AI {
    ...
    public PlayerAction realTimeMinimaxAB(int player, GameState gs, int lookAhead) {
        ...
        maxCT = -1;
        nLeaves = 0;
        MiniMaxResult bestMove = realTimeMinimaxAB(gs, maxplayer, minplayer, alpha, beta,
            gs.getTime() + lookAhead, 0);
        System.out.println(
            "realTimeMinimax: " + bestMove + " in " + (System.currentTimeMillis() - start));
        return bestMove.action;
    }

    public MiniMaxResult realTimeMinimaxAB(GameState gs, int maxplayer, int minplayer, float alpha,
        float beta, int lookAhead, int depth) {
        ...
        if (gs.canExecuteAnyAction(maxplayer)) {
            ...
            for (PlayerAction action_max : actions_max) {
                ...
                MiniMaxResult tmp = realTimeMinimaxAB(gs2, maxplayer, minplayer, alpha, beta,
                    lookAhead, depth + 1);
