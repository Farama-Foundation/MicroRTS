...
class Move {
    ...
    public String toString(ConfigurableScript<?> script) {
        return "choices: " + choices.stream().map(
            (Pair<Integer, Integer> p) -> new Pair<String, Integer>(
                script.choicePointValues[p.m_a].name(), p.m_b)).collect(Collectors.toList())
            + ", player: " + player;
    }
}
...
public abstract class PuppetBase extends AIWithComputationBudget {
    ...
    PuppetBase(int max_time_per_frame, int max_playouts_per_frame, int max_plan_time,
        int max_plan_playouts, int step_playout_time, ConfigurableScript<?> script,
    ...
    boolean planBudgetExpired() {
        return (PLAN_PLAYOUTS >= 0 && totalLeaves >= PLAN_PLAYOUTS) || (PLAN_TIME >= 0
            && totalTime > PLAN_TIME);
    }

    boolean frameBudgetExpired() {
        return (ITERATIONS_BUDGET >= 0 && frameLeaves >= ITERATIONS_BUDGET) || (TIME_BUDGET >= 0
            && frameTime > TIME_BUDGET);
    }
