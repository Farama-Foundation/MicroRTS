...
public class PuppetSearchMCTS extends PuppetBase {
    ...
    public PuppetSearchMCTS(UnitTypeTable utt) {
        this(100, -1, 5000, -1, 100, 100, new RandomBiasedAI(),
            new BasicConfigurableScript(utt, new FloodFillPathFinding()),
            new SimpleSqrtEvaluationFunction3());
    }

    public PuppetSearchMCTS(int max_time_per_frame, int max_playouts_per_frame, int max_plan_time,
        int max_plan_playouts, int step_playout_time, int eval_playout_time, AI policy,
        ...
        super(max_time_per_frame, max_playouts_per_frame, max_plan_time, max_plan_playouts,
            step_playout_time, script, evaluation);
            ...
            if (DEBUG >= 1) {
                System.out.println(
                    "Done. Updating Plan:\n" + currentPlan + " in " + totalTime + " ms, wall time: "
                        + (System.currentTimeMillis() - lastSearchTime) + " ms, playouts: "
                        + totalLeaves);
            }
        }
    }
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        if (DEBUG >= 1) {
            System.out.println("Done. Moves:\n" + root + " in " + totalTime + " ms, wall time: " + (
                System.currentTimeMillis() - lastSearchTime) + " ms, playouts: " + totalLeaves);
        }
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
            ...
            if (DEBUG >= 1) {
                System.out.println(
                    "Restarting after " + (gs.getTime() - lastSearchFrame) + " frames, " + (
                        System.currentTimeMillis() - lastSearchTime) + " ms (" + totalTime
                        + " ms)");
            }
        ...
        if (DEBUG >= 3) {
            System.out.println(
                "Starting MCTS at frame " + gs.getTime() + ", player " + player + " with "
                    + TIME_BUDGET + " ms");
        }
    ...
    @Override
    public AI clone() {
        PuppetSearchMCTS clone = new PuppetSearchMCTS(TIME_BUDGET, ITERATIONS_BUDGET, PLAN_TIME,
            PLAN_PLAYOUTS, STEP_PLAYOUT_TIME, EVAL_PLAYOUT_TIME, policy1.clone(), script.clone(),
            eval);
    ...
    public String statisticsString() {
        return "Average Number of Leaves: " + (allSearches > 0 ? allLeaves / allSearches : "-")
            + ", Average Time: " + (allSearches > 0 ? allTime / allSearches : "-");
    }
    ...
    class Plan {
        ...
        void update(GameState gs, int player) {
            while (valid() && ((gs.getTime() - node.gs.getTime()) > STEP_PLAYOUT_TIME
                || node.bestChild().player() != player)) {
                node = node.bestChild();
            }
        }
