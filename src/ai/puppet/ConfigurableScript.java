...
public abstract class ConfigurableScript<T extends Enum<T>> extends AbstractionLayerAI {
    ...
    public ArrayList<ArrayList<Pair<Integer, Integer>>> getChoiceCombinations(int player,
        GameState gs) {
        ...
        ArrayList<ArrayList<Pair<Integer, Integer>>> combinations = new ArrayList<ArrayList<Pair<Integer, Integer>>>(
            count);
            ...
            for (int choi = 0; choi < o.numOptions(); choi++) {
                for (int k = 0; k < reps[opt + 1]; k++) {
                    ...
                    for (int it = 0; it < cycles; it++) {
                        combinations.get(it * reps[opt] + choi * reps[opt + 1] + k)
                            .add(new Pair<Integer, Integer>(opt, o.getOption(choi)));
                    }
                }
            }
            opt++;
