...
public class PuppetMCTSNode {
    ...
    public PuppetMCTSNode(GameState gs, ConfigurableScript<?> script, float C,
        int nextPlayerInSimultaneousNode, float bound, PuppetMCTSNode parent, Move prevMove,
        int index) {
        this.gs = gs;
        this.script = script;
        this.C = C;
        ...
        actions = script.getChoiceCombinations(toMove(), gs).stream()
            .map(e -> new Move(e, toMove())).toArray(Move[]::new);
    ...
    PuppetMCTSNode selectLeaf(int STEP_PLAYOUT_TIME) throws Exception {
        ...
        if (children.size() < actions.length) {
            ...
            //if first player
            if (prevMove == null) {
                PuppetMCTSNode node = new PuppetMCTSNode(gs, script, C,
                    1 - nextPlayerInSimultaneousNode, evaluation_bound, this, m, children.size());
                children.add(node);
            ...
            } else//second player
            {
                ...
                PuppetMCTSNode node = new PuppetMCTSNode(gs2, script, C,
                    nextPlayerInSimultaneousNode, evaluation_bound, this, null,
                    children.size());//players alternate in 1-2-2-1
                children.add(node);
                return node;
            }
        } else//all children expanded, Bandit policy:
        {
            ...
            for (int child = 0; child < children.size(); child++) {
                ...
                double exploration = Math.sqrt(Math.log(total_visit_count) / visit_count[child]);
