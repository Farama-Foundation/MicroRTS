...
public class PuppetSearchAB extends PuppetBase {
     ...
     * @param mi
     */
    public PuppetSearchAB(int max_time_per_frame, int max_playouts_per_frame, int max_plan_time,
        int max_plan_playouts, int playout_time, ConfigurableScript<?> script,
        EvaluationFunction evaluation) {
        super(max_time_per_frame, max_playouts_per_frame, max_plan_time, max_plan_playouts,
            playout_time, script, evaluation);
    ...
    public void startNewComputation(int player, GameState gs) {
        ...
        stack.clear();
        stack.push(new ABCDNode(new PuppetGameState(gs.clone()), null, -EvaluationFunction.VICTORY,
            EvaluationFunction.VICTORY, 0, MAXPLAYER, null));
        head = stack.peek();
    ...
    public void computeDuringOneGameFrame() throws Exception {
        ...
        do {
            if (DEPTH == 0) {//just started
                ...
                if (DEBUG >= 2) {
                    System.out.println(
                        "ABCD:\n" + lastFinishedHead + " in " + (System.currentTimeMillis()
                            - lastSearchTime) + " ms, leaves: " + totalLeaves + ", depth: "
                            + DEPTH);
                }
                DEPTH += 2;
                stack.push(
                    new ABCDNode(new PuppetGameState(head.gs), null, -EvaluationFunction.VICTORY,
                        EvaluationFunction.VICTORY, 0, MAXPLAYER, null));
                head = stack.peek();
                reached = false;
        ...
        if (searchDone()) {
            if (DEBUG >= 1) {
                System.out
                    .println(ttHits + "/" + ttQueries + " TT, " + ctHits + "/" + ctQueries + " CT");
            }
            stack.clear();
            currentPlan = new Plan(lastFinishedHead);
            if (DEBUG >= 1) {
                System.out.println(
                    "ABCD:\n" + currentPlan + " in " + totalTime + " ms, wall time: " + (
                        System.currentTimeMillis() - lastSearchTime) + " ms, leaves: "
                        + totalLeaves);
            }
        }
    }
    ...
    protected void iterativeABCD(int maxDepth) throws Exception {
        ...
        if (DEBUG >= 2) {
            System.out.println("ABCD at " + head.gs.gs.getTime());
        }
        ...
        while (!stack.isEmpty() && !frameBudgetExpired() && !searchDone()) {
            ...
            if (current.prevMove == null) {//first side to choose move
                if (current.depth == maxDepth || current.gs.gs.gameover()) {//evaluate
                    ...
                    Result result = new Result(parent.nextMoves.last(),
                        eval.evaluate(MAXPLAYER, 1 - MAXPLAYER, current.gs.gs));
                    ...
                    stack.push(new ABCDNode(current.gs, current.nextMoves.next(), current.alpha,
                        current.beta, current.depth + 1, 1 - current.nextPlayerInSimultaneousNode,
                        null));
                    ...
                    if (tt) {
                        TT.store(current.gs, current.best.m, current.best.score, current.alpha,
                            current.beta, maxDepth - current.depth);
                    }
                }
                ...
                if (current.nextMoves.hasNext()) {//check children
                    ...
                    if (gs2 == null) {
                        ...
                        simulate(gsTemp, sc1, sc2, current.prevMove.player, next.player,
                            STEP_PLAYOUT_TIME);

                        gs2 = new PuppetGameState(current.gs, gsTemp, current.depth - 1,
                            current.prevMove, next);
                        if (ct) {
                            CT.store(current.gs, gs2);
                        }
                    }
                    stack.push(
                        new ABCDNode(gs2, null, current.alpha, current.beta, current.depth + 1,
                            current.nextPlayerInSimultaneousNode, null));
                } else {//all children checked, return up
                    stack.pop();
                    ABCDNode parent = stack.peek();
                    parent.setResult(new Result(parent.nextMoves.last(), current.best.score),
                    ...
                    if (tt) {
                        TT.store(current.gs, current.depth, current.prevMove, current.best.m,
                            current.best.score, current.alpha, current.beta,
