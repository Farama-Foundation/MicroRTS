...
public class PlayerAction {
    ...
    /**
     * Represents the resources used by the player action TODO rename the field
     */
    ...
    public boolean equals(Object o) {
        ...
        for (Pair<Unit, UnitAction> p : actions) {
            for (Pair<Unit, UnitAction> p2 : a.actions) {
                if (p.m_a.getID() == p2.m_a.getID() && !p.m_b.equals(p2.m_b)) {
                    return false;
                }
            }
        }
    ...
    /**
     * Returns whether the player has assigned any action different than {@link
     * UnitAction#TYPE_NONE} to any of its units
     *
     * @return
     */
    ...
    /**
     * Returns the number of actions different than {@link UnitAction#TYPE_NONE}
     *
     * @return
     */
    ...
    /**
     * Assign "none" to all the units that need an action and do not have one for the specified
     * duration
     *
     * @param s
        ...
        for (Unit u : pgs.getUnits()) {
            if (u.getPlayer() == pID) {
                if (s.unitActions.get(u) == null) {
                    ...
                    if (!found) {
                        actions.add(new Pair<Unit, UnitAction>(u,
                            new UnitAction(UnitAction.TYPE_NONE, duration)));
                    }
                }
            }
        }
    ...
    /**
     * Returns true if this object passes the integrity check. It fails if the unit is being
     * assigned an action from a player that does not owns it
     *
     * @return
     */
