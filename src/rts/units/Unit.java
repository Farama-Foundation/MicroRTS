...
public class Unit implements Serializable {
    ...
    /**
     * Constructs a unit, specifying with all parameters, including the ID. {@link #next_ID} gets
     * ID+1 if ID >= {@link #next_ID}
     *
     * @param a_ID
     * @param a_player
     * @param a_type
    ...
    /**
     * Creates a unit without specifying its ID. It is automatically assigned from {@link #next_ID},
    ...
    /**
     * Sets the type of this unit. Note: this should not be done lightly. It is currently thought to
     * be used only when the GUI changes the unit type table, and tries to create a clone of the
     * current game state, but changing the UTT.
     *
     * @param a_type
     */
    ...
    /**
     * Changes the unique identifier Note: Do not use this function unless you know what you are
     * doing!
     *
     * @param a_ID
     */
    ...
    /**
     * Returns the index of this unit in a {@link PhysicalGameState} (as it is an 'unrolled
     * matrix')
     *
     * @param pgs
     * @return
    ...
    /**
     * Returns a list of actions this unit can perform in a given game state. An idle action for 10
     * cycles is always generated
     *
     * @param s
     * @return
     */
    ...
    /**
     * Returns a list of actions this unit can perform in a given game state. An idle action for
     * noneDuration cycles is always generated
     *
     * @param s
        ...
        */
        ...
        if (type.canAttack) {
            if (type.attackRange == 1) {
                ...
                if (x < pgs.getWidth() - 1 && uright != null && uright.player != player
                    && uright.player >= 0) {
                    l.add(new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, uright.x, uright.y));
                }
                if (y < pgs.getHeight() - 1 && udown != null && udown.player != player
                    && udown.player >= 0) {
        ...
        if (type.canHarvest) {
            ...
            if (resources > 0) {
                ...
                if (x < pgs.getWidth() - 1 && uright != null && uright.type.isStockpile
                    && uright.player == player) {
                    l.add(new UnitAction(UnitAction.TYPE_RETURN, UnitAction.DIRECTION_RIGHT));
                }
                if (y < pgs.getHeight() - 1 && udown != null && udown.type.isStockpile
                    && udown.player == player) {
    ...
    public void toxml(XMLWriter w) {
        w.tagWithAttributes(this.getClass().getName(),
            "type=\"" + type.name + "\" " + "ID=\"" + ID + "\" " + "player=\"" + player + "\" "
