...
public class PlayerActionGenerator {
    ...
    public PlayerActionGenerator(GameState a_gs, int pID) throws Exception {
        ...
        if (choices.size() == 0) {
            ...
            throw new Exception("Move generator for player " + pID
                + " created with no units that can execute actions! (status: " + a_gs
                .canExecuteAnyAction(0) + ", " + a_gs.canExecuteAnyAction(1) + ")");
        }
    ...
    /**
     * Increases the index that tracks the next action to be returned by {@link
     * #getNextAction(long)}
     *
     * @param startPosition
     */
    ...
    public PlayerAction getNextAction(long cutOffTime) throws Exception {
        ...
        while (moreActions) {
            ...
            if (i == 0) {
                throw new Exception(
                    "Move generator created with no units that can execute actions!");
            }
    ...
    public PlayerAction getRandom() {
        ...
        for (Pair<Unit, List<UnitAction>> unitChoices : choices) {
            List<UnitAction> l = new LinkedList<UnitAction>(unitChoices.m_b);
            Unit u = unitChoices.m_a;
    ...
    public String toString() {
        ...
        for (Pair<Unit, List<UnitAction>> choice : choices) {
            ret.append("  (").append(choice.m_a).append(",").append(choice.m_b.size())
                .append(")\n");
        }
