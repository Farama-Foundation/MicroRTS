...
public class GameState {
    ...
    /**
     * Returns true if there is no unit in the specified position and no unit is executing an action
     * that will use that position
     *
    ...
    public boolean free(int x, int y) {
        ...
        for (UnitActionAssignment ua : unitActions.values()) {
            if (ua.action.type == UnitAction.TYPE_MOVE
                ...
                Unit u = ua.unit;
                if (ua.action.getDirection() == UnitAction.DIRECTION_UP && u.getX() == x
                    && u.getY() == y + 1) {
                    return false;
                }
                if (ua.action.getDirection() == UnitAction.DIRECTION_RIGHT && u.getX() == x - 1
                    && u.getY() == y) {
                    return false;
                }
                if (ua.action.getDirection() == UnitAction.DIRECTION_DOWN && u.getX() == x
                    && u.getY() == y - 1) {
                    return false;
                }
                if (ua.action.getDirection() == UnitAction.DIRECTION_LEFT && u.getX() == x + 1
                    && u.getY() == y) {
                    return false;
                }
            }
    ...
    /**
     * Returns a boolean array with true if there is no unit in the specified position and no unit
     * is executing an action that will use that position
     *
     * @return
     */
    ...
    /**
     * Returns whether the cell is observable. For fully observable game states, all the cells are
     * observable.
     *
     * @param x
     * @param y
                ...
                if (!uaa.action.resourceUsage(uaa.unit, pgs).consistentWith(ru, this)) {
                    ...
                    if (uaa.time == time) {
                        ...
                        switch (utt.getMoveConflictResolutionStrategy()) {
                            default:
                                System.err.println(
                                    "Unknown move conflict resolution strategy in the UnitTypeTable!: "
                                        + utt.getMoveConflictResolutionStrategy());
                                System.err
                                    .println("Defaulting to MOVE_CONFLICT_RESOLUTION_CANCEL_BOTH");
                        ...
                        if (cancel_old) {
                            ...
                            uaa.action = new UnitAction(UnitAction.TYPE_NONE,
                                Math.min(duration1, duration2));
                        }
                        if (cancel_new) {
                            //                                System.out.println("New action canceled: " + p.m_a.getID() + ", " + p.m_b);
                            p = new Pair<>(p.m_a, new UnitAction(UnitAction.TYPE_NONE,
                                Math.min(duration1, duration2)));
                        }
                    } else {
                        ...
                        System.err
                            .println("  Resources: " + uaa.action.resourceUsage(uaa.unit, pgs));
                        System.err
                            .println(p.m_a + " assigned action " + p.m_b + " at time " + time);
                        System.err.println("  Resources: " + ru);
                        System.err.println(
                            "Player resources: " + pgs.getPlayer(0).getResources() + ", " + pgs
                                .getPlayer(1).getResources());
                        System.err.println(
                            "Resource Consistency: " + uaa.action.resourceUsage(uaa.unit, pgs)
                                .consistentWith(ru, this));

                        try {
                            throw new Exception(
                                "dummy");   // just to be able to print the stack trace
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public boolean isUnitActionAllowed(Unit u, UnitAction ua) {
        ...
        if (ua.getType() == UnitAction.TYPE_MOVE) {
            ...
            if (x2 < 0 || y2 < 0 || x2 >= getPhysicalGameState().getWidth()
    ...
    /**
     * Returns the time the next unit action will complete, or current time if a player can act
     *
     * @return
     */
    ...
    public String toString() {
        ...
        for (Unit u : unitActions.keySet()) {
            ...
            if (ua == null) {
                ...
                tmp.append("    ").append(u).append(" -> ").append(ua.time).append(" ")
                    .append(ua.action).append("\n");
            }
        }
        tmp.append(pgs);
        return tmp.toString();
    ...
    /**
     * Dumps this state to a XML file. It can be reconstructed later (e.g. with {@link
     * #fromXML(String, UnitTypeTable)}
     *
     * @param path
     */
