...
/**
 * A partially observable game state. It is associated with a player to check whether it is able to
 * observe the map portion
 *
 * @author santi
 */
public class PartiallyObservableGameState extends GameState {
    ...
    public boolean observable(int x, int y) {
        for (Unit u : pgs.getUnits()) {
            if (u.getPlayer() == player) {
                double d = Math
                    .sqrt((u.getX() - x) * (u.getX() - x) + (u.getY() - y) * (u.getY() - y));
